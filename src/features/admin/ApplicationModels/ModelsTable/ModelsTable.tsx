...
const ModelsTable: React.FC<TProps> = ({isEdit, pageData, onChangeRowsPerPage, onChangePage, data, setData, order, setOrder}) => {
    const {isLoading, allModelsOptions, modelsPagination} = useSelector((state: RootState) => state.globalVehicles);
    ...
    const onModelChange = (el: IGlobalModel) => (e: React.ChangeEvent<{}>, option: IGlobalModel|null) => {
        setData(prev => {
            const itemToChange = prev.find(item => item.id === el.id)
            if (itemToChange) {
    ...
    const RowData: TableRowDataType<IGlobalModel>[] = [
        ...
        },
        {
            ...
            val: el => !isEdit
                ? el.accepted && el.parent?.vinModel ? el.parent?.vinModel : " "
