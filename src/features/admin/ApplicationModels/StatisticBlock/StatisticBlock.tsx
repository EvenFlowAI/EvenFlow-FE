...
const StatisticBlock = () => {
  const { modelsStatistic, isLoading } = useSelector(
  ...
  return isLoading ? (
    ...
    <Wrapper>
      <Statistic>
        ...
        {modelsStatistic ? (
          <div className="value">
            {modelsStatistic?.confirmed ?? 0} (
            {getPercentage(modelsStatistic)?.confirmed}%)
          </div>
        ) : null}
      </Statistic>
      <Statistic>
        ...
        {modelsStatistic ? (
          <div className="value">
            {modelsStatistic?.overriden ?? 0} (
            {getPercentage(modelsStatistic)?.overriden}%)
          </div>
        ) : null}
      </Statistic>
      <Statistic>
        ...
        {modelsStatistic ? (
          <div className="value">
            {modelsStatistic?.notReviewed ?? 0} (
            {getPercentage(modelsStatistic)?.notReviewed}%)
          </div>
        ) : null}
      </Statistic>
    </Wrapper>
