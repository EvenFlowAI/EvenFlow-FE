...
const RecallAppointments: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<TNotificatonsProps>>
> = ({ setChangesState, onClose }) => {
  const { usersShort, loading } = useSelector(
    (state: RootState) => state.employees,
  );
  const { recallNotifications, isLoading } = useSelector(
    (state: RootState) => state.notifications,
  );
  const [currentEmployee, setCurrentEmployee] = useState<IAdvisorShort | null>(
    null,
  );
  const [recallData, setRecallData] = useState<TSCNotifications | null>(
    initialSCNotifications,
  );
  ...
  useEffect(() => {
    ...
    if (recallNotifications || recallData) {
      if (currentEmployee) {
        ...
        const employeesListIsDifferent =
          recallNotifications?.employees?.find(
            (el) => !recallData?.employees?.includes(el),
          ) ||
          recallData?.employees?.find(
            (el) => !recallNotifications?.employees?.includes(el),
          );
        if (
          Boolean(recallData?.isActive) !==
          Boolean(recallNotifications?.isActive)
        ) {
          changesAreSaved = false;
        } else if (
          recallData?.employees?.length !==
          recallNotifications?.employees?.length
        ) {
          changesAreSaved = false;
    ...
    setChangesState((prevState) => ({
      ...prevState,
      recallNotificationsSaved: changesAreSaved,
    }));
  ...
  useEffect(() => {
    const selected = usersShort.filter((el) =>
      recallData?.employees?.includes(el.id),
    );
      ...
      if (prevState) {
        return {
          ...
          employees: prevState?.employees
            ? prevState?.employees.filter((el) => el !== id)
            : [],
        };
      } else {
        return prevState;
