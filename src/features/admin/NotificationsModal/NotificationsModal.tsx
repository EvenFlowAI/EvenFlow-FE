...
> = (props) => {
  ...
  const onClose = () => {
    if (Object.values(changesState).some((value) => !value)) {
      showError(
        "Please save your entries before closing the service center notifications window.",
      );
    } else {
      props.onClose();
    }
  ...
  const handleTabChange = (e: React.ChangeEvent<{}>, tab: string) => {
    if (checkIfChangesSaved(currentTab)) {
      ...
      showError(
        "Please save your entries before switching to a different notification tab.",
      );
    }
  };
