...
const ServiceCenterAppointments: React.FC<
...
> = ({ setChangesState, onClose }) => {
  ...
  const { scNotifications, isLoading } = useSelector(
    (state: RootState) => state.notifications,
  );
  const [currentEmployee, setCurrentEmployee] = useState<IAdvisorShort | null>(
    null,
  );
  const [scData, setScData] = useState<TSCNotifications | null>(
  ...
  useEffect(() => {
    ...
    if (scNotifications || scData) {
      if (currentEmployee) {
        ...
        const employeesListIsDifferent =
          scNotifications?.employees?.find(
            (el) => !scData?.employees?.includes(el),
          ) ||
          scData?.employees?.find(
            (el) => !scNotifications?.employees?.includes(el),
          );
        const listsLengthIsDifferent =
          Number(scData?.employees?.length) !==
          Number(scNotifications?.employees?.length);
    ...
    setChangesState((prevState) => ({
      ...prevState,
      scNotificationsSaved: changesAreSaved,
    }));
  ...
  useEffect(() => {
    const selected = usersShort.filter((el) =>
      scData?.employees?.includes(el.id),
    );
      ...
      <div className={classes.tabWrapper}>
        ...
        <div className={classes.switcherWrapper}>
          <p className={classes.notificationsLabel}>
            on/off Service center and Service Books notifications
          </p>
          <Switch
        ...
        <div className={classes.selectWrapper}>
          <Autocomplete
            ...
            renderInput={autocompleteRender({
              ...
              error: Boolean(
                currentEmployee &&
                  !scData?.employees?.includes(currentEmployee?.id) &&
                  formChecked,
              ),
            })}
          />
