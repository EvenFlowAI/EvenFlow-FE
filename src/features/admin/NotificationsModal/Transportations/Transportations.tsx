...
> = ({ setChangesState, changesState, onClose }) => {
  ...
  const [allTransportationData, setAllTransportationData] =
    useState<TTransportationNotifications | null>(
      initialTransportationNotifications,
    );
  ...
  const currentTransportationData = useMemo(() => {
    return allTransportationData?.transportationOptions?.find(
      (el) => el.id === selectedTransportation?.id,
    );
  }, [allTransportationData, selectedTransportation]);

  const inactiveOptionsIds = useMemo(
    () => options.filter((op) => op.state === 0).map((el) => el.id),
    [options],
  );
  ...
  const activeOptionsNotifications = useMemo(() => {
    if (transportationNotifications?.transportationOptions) {
      return transportationNotifications?.transportationOptions.filter(
        (el) => !inactiveOptionsIds.includes(el.id),
      );
    }
    return [];
  ...
  useEffect(() => {
    ...
    if (currentEmployee) {
      ...
      if (
        ...
        changesSaved = checkTransportationAreTheSame(
  ...
  const setInitialTransportation = useCallback(() => {
    ...
    if (activeOptionsNotifications?.length) {
      const option = options.find(
        (el) => el.id === activeOptionsNotifications[0].id,
      );
  ...
  const setInitialEmployees = useCallback(() => {
    if (currentTransportationData?.usersList) {
      const selected = usersShort.filter((el) =>
        currentTransportationData?.usersList?.includes(el.id),
      );
  ...
  const onTransportationChange = (
    e: ChangeEvent<{}>,
    value: ITransportationOptionFull | null,
  ) => {
    setFormChecked(false);
    ...
    if (transportationData) {
      const selected = usersShort.filter((el) =>
        transportationData.usersList?.includes(el.id),
      );
      setSelectedEmployees(selected);
    } else {
      if (value) {
  ...
  const onCancel = () => {
    ...
    if (changesState?.transportationNotificationsSaved) {
      ...
      askConfirm({
        ...
        content: (
          <span>
            By clicking Cancel, your entries across all Transportations will not
            be saved.
            <br />
  ...
  const sendRequest = () => {
    if (allTransportationData) {
      const data: TTransportationNotifications = {
        ...allTransportationData,
        transportationOptions:
          allTransportationData?.transportationOptions.filter(
            (el) => !inactiveOptionsIds.includes(el.id),
          ),
      };
      if (selectedSC && allTransportationData)
        dispatch(
          updateTransportationNotifications(
            selectedSC.id,
            data,
      ...
      if (currentTransportationData) {
        const updated = {
          ...
          usersList: currentTransportationData.usersList
            ? Array.from(
                new Set([
                  ...currentTransportationData.usersList,
                  currentEmployee.id,
                ]),
              )
            : [currentEmployee.id],
        ...
        const data = allTransportationData?.transportationOptions.filter(
          (el) => el.id !== currentTransportationData.id,
        );
        if (data) {
          setAllTransportationData((prev) =>
            prev
  ...
  const handleSwitch = () => {
    setFormChecked(false);
    setAllTransportationData((prev) =>
      prev ? { ...prev, isActive: prev ? !prev.isActive : true } : prev,
    );
  };
  ...
  const deleteEmployee = (id: string) => {
    ...
    if (currentTransportationData && currentTransportationData.usersList) {
      const updated = {
        ...currentTransportationData,
        usersList: currentTransportationData.usersList.filter(
          (el) => el !== id,
        ),
      };
      const data = allTransportationData?.transportationOptions.filter(
        ...
        {loading || isLoading || isSaving ? (
          ...
          <React.Fragment>
            ...
            <div className={classes.switcherWrapper}>
              <p className={classes.notificationsLabel}>
            ...
            <div className={classes.selectWrapper}>
              <Autocomplete
                ...
                renderInput={autocompleteRender({
                  ...
                  error: Boolean(
                    currentEmployee &&
                      !currentTransportationData?.usersList?.includes(
                        currentEmployee?.id,
                      ) &&
                      formChecked,
                  ),
