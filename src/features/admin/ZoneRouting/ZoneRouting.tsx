...
const ZoneRouting = () => {
  ...
  const { zonesRouting, isLoading } = useSelector(
  ...
  useEffect(() => {
    setInitialZones(
      [1, 2, 3, 4, 5, 6, 0].map((item) => {
        const existingDay = zonesRouting.find(
          (el) => el.dayOfWeek.toString() === item.toString(),
        );
  ...
  const onCheckboxChange =
    (zoneId: number, dayOfWeek: EDaysFromMonday) =>
    (e: any, checked: boolean) => {
      const dayOfWeekData = initialZones.find(
      ...
      if (dayOfWeekData && selectedSC) {
        ...
        if (checked) {
          ...
          updatedData = {
            ...updatedData,
            geographicZoneIds: updatedData.geographicZoneIds.filter(
              (el) => el !== zoneId,
            ),
          };
        }
        const data = zonesRouting
  ...
  const getRowData = (): TableRowDataType<IZonesRoutingByDay>[] => {
    ...
    const zonesData: TableRowDataType<IZonesRoutingByDay>[] = zones.map(
      (item) => {
        return {
          header: item.name,
          width: 100,
          val: (el) => (
            <Checkbox
              ...
              checked={Boolean(
                el.geographicZoneIds.find((zoneId) => item.id === zoneId),
              )}
