...
const CapacityAdvisors = () => {
  ...
  const { advisors, isLoading } = useSelector(
  ...
  const onChange = ({
    ...
    setData((prev) => {
      ...
      if (itemToChange) {
        const serviceBook = itemToChange.capacityPerServiceBook.find(
          (el) => el.id === +serviceBookId,
        );
        const serviceCenter = itemToChange.capacityPerServiceBook.find(
          (el) => !el.id,
        );
        if (serviceBook) {
          itemToChange = {
  ...
  const checkIsValid = (): boolean => {
    return !data.find((el) =>
      el.capacityPerServiceBook.find((item) => item.value < 0),
    );
  };
  ...
  const ServiceRowsData: TableRowDataType<IAdvisorCapacity>[] =
    shortPodsList.map((serviceBook) => ({
      header: serviceBook.name,
      width: 120,
      val: (el) => (
        <EditableTableCell
          value={
            el.capacityPerServiceBook.find(
              (item) => item.id && item.id === serviceBook.id,
            )?.value ?? "0"
          }
          name={
            serviceBook?.id
              ? `${el.employeeId}/${serviceBook.id}`
              : `${el.employeeId}`
          }
          onChange={onChange}
          isEdit={isEdit}
  ...
  const RowData: TableRowDataType<IAdvisorCapacity>[] = [
    ...
    },
    {
      ...
      val: (el) => (
        <EditableTableCell
          value={
            el.capacityPerServiceBook.find((item) => !item.id)?.value ?? "0"
          }
          name={`${el.employeeId}`}
          onChange={onChange}
          isEdit={isEdit}
