...
export const EditEmailRequirementModal: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<DialogProps>>
> = ({ onClose, ...props }) => {
  const { emailRequirement, isEmailRequirementLoading } = useSelector(
  ...
  const onSave = () => {
    if (selectedSC && data) {
      dispatch(
        updateEmailRequirement(selectedSC.id, data, showError, onSuccess),
      );
    }
  };
            ...
            <TableHead>
              <TableRow>
                <TableCellWithPadding
                  style={{ color: "#9FA2B4", fontSize: 12 }}
                  align="left"
                >
                  Booking Flow
                </TableCellWithPadding>
              ...
              <TableRow>
                ...
                <TableCellWithPadding align="center">
                  {!data?.adminAndEmployeesEnabled ? (
                    <RadioButtonChecked htmlColor="#3855F3" cursor="pointer" />
                  ) : (
                    <RadioButtonUnchecked
                      htmlColor="#DADADA"
                      cursor="pointer"
                      onClick={() =>
                        onChange("adminAndEmployeesEnabled", false)
                      }
                    />
                  )}
                </TableCellWithPadding>
                ...
                <TableCellWithPadding align="center">
                  {!data?.customerSelfServiceEnabled ? (
                    ...
                    <RadioButtonUnchecked
                      ...
                      cursor="pointer"
                      onClick={() =>
                        onChange("customerSelfServiceEnabled", false)
                      }
                    />
                  )}
                </TableCellWithPadding>
                <TableCellWithPadding align="center">
                  {data?.customerSelfServiceEnabled ? (
                    ...
                    <RadioButtonUnchecked
                      ...
                      cursor="pointer"
                      onClick={() =>
                        onChange("customerSelfServiceEnabled", true)
                      }
                    />
                  )}
                </TableCellWithPadding>
