...
const EditCustomerConsentModal: React.FC<
  DialogProps & { consentId: number | undefined }
> = ({ open, onClose, consentId }) => {
  const { advisorsList } = useSelector(
  ...
  const { makesModels } = useSelector(
    ({ vehicleDetails }: RootState) => vehicleDetails,
  );
  const { mobileZonesShort } = useSelector(
    ({ mobileService }: RootState) => mobileService,
  );
  const { svZonesShort } = useSelector(
    ({ serviceValet }: RootState) => serviceValet,
  );
  ...
  const { slotRange } = useSelector(
    ({ slotScoring }: RootState) => slotScoring,
  );
  ...
  const startTimeError = useMemo(() => {
    return (
      formIsChecked &&
      (dayjs(form.appointmentTimeTo, timeSpanString).isSameOrBefore(
        dayjs(form.appointmentTimeFrom, timeSpanString),
        "minute",
      ) ||
        dayjs(form.appointmentTimeFrom, timeSpanString).isBefore(
          dayjs(slotRange?.start, timeSpanString),
          "minute",
        ) ||
        dayjs(form.appointmentTimeFrom, timeSpanString).isAfter(
          dayjs(slotRange?.end, timeSpanString),
          "minute",
        ))
    );
  ...
  const endTimeError = useMemo(() => {
    return (
      ...
      (dayjs(form.appointmentTimeTo, timeSpanString).isSameOrBefore(
        ...
        dayjs(form.appointmentTimeTo, timeSpanString).isBefore(
          dayjs(slotRange?.start, timeSpanString),
          "minute",
        ) ||
        dayjs(form.appointmentTimeTo, timeSpanString).isAfter(
          dayjs(slotRange?.end, timeSpanString),
          "minute",
        ))
    );
      ...
      if (currentConsent) {
        setForm(() => {
          ...
          return {
            ...
            makes: makesModels.filter((el) =>
              currentConsent.makeIds?.includes(el.id),
            ),
            models: makesModels
              .map((el) => el.models)
              .flat(1)
            ...
            advisors: advisorsList.filter((el) =>
              currentConsent.advisorIds?.includes(el.id),
            ),
  ...
  const checkIsValid = () => {
    let isValid = Boolean(
      form.message.length && form.name.length && form.title.length,
    );
    if (
      form.modelYearFrom &&
      form.modelYearTo &&
    ...
    if (form.modelYearFrom && !form.modelYearTo) {
      isValid = false;
      showError(
        '"Year To" must not be empty. Either select "Year To" or remove "Year From" value',
      );
    }
    if (form.modelYearTo && !form.modelYearFrom) {
      isValid = false;
      showError(
        '"Year From" must not be empty. Either select "Year From" or remove "Year To" value',
      );
    }
    if (form.serviceValetZones.length && form.mobileServiceZones.length) {
      isValid = false;
      showError(
        '"Service Valet Zones" cannot be configured with "Mobile Service Zones"',
      );
    }
    ...
    if (form.mobileServiceZones.length) {
      if (form.advisors.length) {
        isValid = false;
        showError(
          '"Service Advisors" cannot be configured with "Mobile Service Zones"',
        );
      }
      if (form.transportationOptions.length) {
        isValid = false;
        showError(
          '"Transportation Options" cannot be configured with "Mobile Service Zones"',
        );
      }
    }
    return isValid;
  ...
  const onSave = () => {
    ...
    if (checkIsValid() && selectedSC) {
      ...
      if (currentConsent) {
        const updateData: ICustomerConsentById = {
          ...data,
    ...
    (e: ChangeEvent<{}>, value: IMakeExtended[]) => {
      ...
      setForm((prev) => ({
        ...
        models: prev.models.filter((item) =>
          value.find((make) =>
            make.models.find((model) => model.id === item.id),
          ),
        ),
      }));
    },
  ...
  const onAutocompleteChange =
    (name: keyof TForm) => (e: ChangeEvent<{}>, value: string | null) => {
      setFormIsChecked(false);
  ...
  const onCustomerTypeChange = (e: ChangeEvent<{}>, value: TOption | null) => {
    ...
    setForm((prev) => ({
      ...prev,
      customerType: value ? (value.value as EUserType) : null,
    }));
  };
  ...
  const onDayOfWeekChange = (e: ChangeEvent<{}>, value: TOption[]) => {
    ...
    setForm((prev) => ({
      ...prev,
      daysOfWeek: value.map(({ value }) => value as EDay),
    }));
  };
  ...
  const onTransportationsChange = (
    e: ChangeEvent<{}>,
    value: TTransportationShort[],
  ) => {
    setFormIsChecked(false);
    setForm((prev) => ({ ...prev, transportationOptions: value }));
  };

  const onMobileZonesChange = (
    e: ChangeEvent<{}>,
    value: TGeographicZoneShort[],
  ) => {
    setFormIsChecked(false);
    setForm((prev) => ({ ...prev, mobileServiceZones: value }));
  };

  const onSvZonesChange = (
    e: ChangeEvent<{}>,
    value: TGeographicZoneShort[],
  ) => {
    setFormIsChecked(false);
            ...
            <Grid container spacing={3} style={{ marginBottom: 36 }}>
              ...
              <Grid item xs={12} sm={12} md={6}>
                <Autocomplete
                  ...
                  isOptionEqualToValue={(o, v) => o.id === v.id}
                  value={form.advisors}
                  renderOption={autocompleteOptionsRender(
                ...
                <Autocomplete
                  ...
                  value={
                    customerTypeOptions.find(
                      (el) => (el.value as EUserType) === form.customerType,
                    ) ?? null
                  }
            ...
            <Grid container spacing={3} style={{ marginBottom: 36 }}>
              ...
              <Grid item xs={12}>
                <Autocomplete
                  ...
                  isOptionEqualToValue={(o, v) => o.value === v.value}
                  renderOption={autocompleteOptionsRender((e) => e.name)}
                  value={dayOfWeekOptions.filter((el) =>
                    form.daysOfWeek.includes(el.value as EDay),
                  )}
              ...
              <Grid item xs={12} sm={6}>
                ...
                <TimeSelect
                  ...
                  onChange={(value) =>
                    onAppointmentTimeChange(value, "appointmentTimeFrom")
                  }
                />
              </Grid>
              <Grid item xs={12} sm={6}>
                ...
                <TimeSelect
                  ...
                  onChange={(value) =>
                    onAppointmentTimeChange(value, "appointmentTimeTo")
                  }
                />
              </Grid>
            </Grid>
