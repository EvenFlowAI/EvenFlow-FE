...
const WaitListSlotSettingsModal: React.FC<
...
> = (props) => {
  ...
  const [rolloverDescriptionText, setRolloverDescriptionText] =
    useState<string>("");
  ...
  useEffect(() => {
    if (waitListSettings && props.open) {
      ...
      setRolloverDescriptionText(
        waitListSettings.rolloverDescriptionText ?? "",
      );
  ...
  const onSlotTextColorChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.value.match(/^[a-zA-Z0-9]*$/)) {
      ...
      showError(
        "Appointment Slot Text Color must consist letters and digits only",
      );
    }
  };
  ...
  const onSlotBoxColorChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.value.match(/^[a-zA-Z0-9]*$/)) {
      ...
      showError(
        "Appointment Slot Background Color must consist letters and digits only",
      );
    }
  };

  const onSlotTextChange = useCallback(
