...
> = ({ setEdit, isEdit, editingElement }) => {
  ...
  const handleSave = () => {
    if (selectedSC && editingElement) {
      const items = [...slots1, ...slots2].map((item) => ({
        ...item,
        amount: +item.amount,
      }));
      const data = {
    ...
    <div className={classes.wrapper}>
      <div className={classes.titleLine}>
        <div className={classes.dayName}>
          {editingElement
            ? dayjs().set("day", editingElement?.day).format("dddd")
            : ""}
        </div>
        <div className={classes.title}>
          Enter The Unplanned Demand By Appointment Slot
        </div>
        <div>
          <SaveEditBlock
            onSave={handleSave}
