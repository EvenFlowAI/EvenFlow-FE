...
export const OfferModal: React.FC<
  React.PropsWithChildren<
    React.PropsWithChildren<DialogProps<IOffer> & { archive?: boolean }>
  >
> = ({ onAction, archive, payload, ...props }) => {
  ...
  const handleSegmentsSelect = (
    e: any,
    value: TEnumMap<ECustomerSegment>[],
  ) => {
    setFormIsChecked(false);
    if (
      form.customerSegments.find(
        (d) => d.id === ECustomerSegment.All && value.length > 1,
      )
    ) {
      setForm({
        ...form,
        customerSegments: value.filter((s) => s.id !== ECustomerSegment.All),
      });
  ...
  const handleDOWSelect = (e: any, value: TEnumMap<EDayOfWeek>[]) => {
    setFormIsChecked(false);
    if (
      form.dayOfWeek.find((d) => d.id === EDayOfWeek.EveryDay) &&
      value.length > 1
    ) {
      setForm({
        ...form,
        dayOfWeek: value.filter((e) => e.id !== EDayOfWeek.EveryDay),
      });
  ...
  const handleChangeDateTime =
    (name: keyof TOfferForm) => (date: TParsableDate) => {
      setFormIsChecked(false);
      ...
      if (checkIsValid()) {
        ...
        try {
          const data: IOfferForm = {
            ...
            timeOfDay: {
              start: dayjs(form.timeOfDayFrom, time12HourSeconds).format(
                timeSpanString,
              ),
              end: dayjs(form.timeOfDayTo, time12HourSeconds).format(
                timeSpanString,
              ),
            },
