...
const ServiceBooksTable = () => {
  const { summary, podsLoading } = useSelector(
  ...
  const onSave = () => {
    ...
    const { wrongNumbers, errors } = findMissingNumbers(
      indexes,
      currentData.length,
    );
    if (!wrongNumbers.length) {
      ...
      selectedSC &&
        dispatch(
          setPodsOrderIndex(selectedSC?.id, data, showError, clearState),
        );
    } else {
  ...
  const onCancel = () => {
    clearState();
    setCurrentData(
      summary.map((el) => ({ ...el, prevOrder: el.orderIndex ?? 0 })),
    );
  };
      ...
      if (Number.isInteger(+e.target.value)) {
        setCurrentData((prev) => {
          const itemToUpdate = prev.find(
            (el) => el.serviceBookId === serviceBookId,
          );
          if (itemToUpdate) {
            const updated = { ...itemToUpdate, orderIndex: +e.target.value };
            return [
              ...prev.filter((item) => item.serviceBookId !== serviceBookId),
              updated,
  ...
  const rowData: TableRowDataTypeResp<IPodSummary>[] = [
        ...
        isEdit ? (
          <StyledField
            ...
            error={
              isChecked &&
              (el.orderIndex
                ? wrongOrderIndexes.includes(el.orderIndex)
                : !el.orderIndex)
            }
    ...
    },
    {
      header: "Op Codes",
      val: (el) =>
        el.options.includes(EPodSummaryOption.OpsCodes) ? (
          <Checked />
        ) : (
          <Unchecked />
        ),
    ...
    },
    {
      header: "Make",
      val: (el) =>
        el.options.includes(EPodSummaryOption.Make) ? (
          <Checked />
        ) : (
          <Unchecked />
        ),
    ...
    },
    {
      header: "Model",
      val: (el) =>
        el.options.includes(EPodSummaryOption.Model) ? (
          <Checked />
        ) : (
          <Unchecked />
        ),
    ...
    },
    {
      header: "Mileage",
      val: (el) =>
        el.options.includes(EPodSummaryOption.Mileage) ? (
          <Checked />
        ) : (
          <Unchecked />
        ),
    ...
    },
    {
      header: "Engine Type",
      val: (el) =>
        el.options.includes(EPodSummaryOption.EngineType) ? (
          <Checked />
        ) : (
          <Unchecked />
        ),
    ...
    },
    {
      ...
      val: (el) =>
        el.options.includes(EPodSummaryOption.ServiceValet) ? (
          <Checked />
        ) : (
          <Unchecked />
        ),
    ...
    },
    {
      ...
      val: (el) =>
        el.options.includes(EPodSummaryOption.MobileService) ? (
          <Checked />
        ) : (
          <Unchecked />
        ),
    ...
    },
    {
      ...
      val: (el) =>
        el.options.includes(EPodSummaryOption.TransportOptions) ? (
          <Checked />
        ) : (
          <Unchecked />
        ),
    ...
    },
    {
      header: "Advisors",
      val: (el) =>
        el.options.includes(EPodSummaryOption.Advisors) ? (
          <Checked />
        ) : (
          <Unchecked />
        ),
  ...
  const handleRemove = async () => {
    if (!currentItem) {
      ...
      try {
        await dispatch(
          removePod(currentItem.serviceBookId, selectedSC?.id, showError),
        );
        setCurrentItem(null);
      } catch (e) {
        showError(e);
