...
export const AddHolidayModal: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<DialogProps<IHoliday>>>
> = ({ onAction, payload, ...props }) => {
  ...
  const handleSave = async () => {
    if (!selectedSC) {
      ...
      try {
        if (payload) {
          await Api.call(Api.endpoints.Holidays.Update, {
            data,
          ...
          await Api.call(Api.endpoints.Holidays.Create, { data }).then(
            (res) => {
  ...
  return (
    <BaseModal {...props} width={600}>
      <DialogTitle onClose={props.onClose}>
        {payload ? "Edit" : "Add"} Holiday
      </DialogTitle>
      <DialogContent>
        <HolidayForm
