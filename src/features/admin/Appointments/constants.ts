...
export const AppointmentsColumns: TableRowDataType<IAppointment>[] = [
  {
    ...
    required: true,
    val: (el) =>
      el.dateTime ? dayjs.utc(el.dateTime).format("MMMM D, YYYY") : "",
    orderId: "date",
    width: 150,
  },
  {
  ...
  },
  {
    ...
    required: true,
    val: (el) =>
      el.dateTime ? dayjs.utc(el.dateTime).format(time12HourFormat) : "",
    width: 100,
  },
  {
    ...
    required: true,
    val: (el) =>
      el.customerInformation?.fullName ?? "DMS missing customer information",
    orderId: "fullName",
  },
  {
    header: "Vehicle",
  ...
  { header: "Scheduler", val: (el) => `${el.scheduler?.fullName ?? ""}` },
  {
    ...
    required: true,
    val: (el) =>
      typeof el.reportingStatus !== "undefined" &&
      Number.isInteger(el.reportingStatus)
  ...
  },
  {
    ...
    val: (el) =>
      el.createdDateTime
        ? dayjs.utc(el.createdDateTime).format("ddd, MMMM D, YYYY")
        : "",
    orderId: "requestDate",
  },
];
export const allColumns = AppointmentsColumns.map((el) => el.header.toString());
export const requiredColumns = AppointmentsColumns.filter(
  (el) => el.required,
).map((el) => el.header.toString());
