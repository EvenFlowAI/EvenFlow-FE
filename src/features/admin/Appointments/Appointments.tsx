...
export const Appointments = () => {
  ...
  const [selectedColumns, setSelectedColumns] =
    useState<string[]>(requiredColumns);
  const {
    isOpen: isListOpen,
    onClose: onListClose,
    onOpen: onListOpen,
  ...
  const getAppointments = useCallback(() => {
    if (!filters.dateTo && !filters.dateFrom) {
      ...
      if (filters.dateTo && filters.dateFrom) {
        if (
          Math.round(
            dayjs(filters.dateTo).diff(filters.dateFrom) /
              (1000 * 60 * 60 * 24),
          ) > 90
        ) {
          showError(
          ...
          if (
            ...
            const isServiceBookServiceCenter = Boolean(
              filters.serviceBook && !serviceBookId,
            );
            const data: IAppointmentsRequest = {
              ...
              startDate: dayjs(filters.dateFrom).add(
                dayjs(filters.dateFrom).utcOffset(),
                "minute",
              ),
              endDate: dayjs(filters.dateTo).add(
                dayjs(filters.dateTo).utcOffset(),
                "minute",
              ),
            ...
            if (filters.technician)
              data.technicianDmsId = filters.technician.dmsId;
  ...
  const onChangePage = useCallback(
    (
      ...
      setFilters((prev) => ({
        ...prev,
        pageData: { ...prev.pageData, pageIndex },
      }));
    },
    [],
  );
  ...
  const onChangeRowsPerPage = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>): void => {
      setFilters((prev) => ({
        ...prev,
        pageData: { pageIndex: 0, pageSize: +e.target.value },
      }));
    },
    [],
  );
