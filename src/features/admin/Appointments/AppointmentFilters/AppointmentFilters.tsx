...
export const AppointmentFilters: React.FC<TAppointmentFilterProps> = ({
  ...
  const rangeIsWrong = useMemo(() => {
    return (
      dateTo &&
      dateFrom &&
      Math.round(dayjs(dateTo).diff(dateFrom) / (1000 * 60 * 60 * 24)) > 90
    );
  }, [dateTo, dateFrom]);
  ...
  const handleDateChange =
    (field: "dateFrom" | "dateTo") => (date: TParsableDate) => {
      setFilters((prev) => {
        if (
          field === "dateFrom" &&
          (dayjs(date).isAfter(prev.dateTo) || !prev.dateTo)
        ) {
          return {
            ...prev,
            [field]: dayjs(date),
          ...
          field === "dateTo" &&
          (dayjs(date).isBefore(prev.dateFrom) || !prev.dateFrom)
        ) {
          return {
            ...prev,
            [field]: dayjs(date),
  ...
  const handleType = (
    ...
    setFilters((prev) => ({
      ...prev,
      dateRangeFilterBy:
        value === "AppointmentDate" ? EDate.AppointmentDate : EDate.CreatedDate,
      pageData: { ...prev.pageData, pageIndex: 0 },
    }));
  };
    ...
    >
      <RadioBlock>
        ...
        <RadioGroup
          ...
          name="countType"
          value={
            dateRangeType === EDate.AppointmentDate
              ? "AppointmentDate"
              : "CreatedDate"
          }
          onChange={handleType}
      ...
      >
          ...
          <CustomDatePicker
            ...
            InputProps={{
              ...
              endAdornment: dateFrom ? (
                <IconButton
                  onClick={(e) => handleClear(e, "dateFrom")}
                  size="large"
                >
                  <Clear />
                </IconButton>
        ...
        <Grid item xs={12} sm={3} key="datepickerTo">
          <CustomDatePicker
            ...
            InputProps={{
              ...
              endAdornment: dateTo ? (
                <IconButton
                  onClick={(e) => handleClear(e, "dateTo")}
                  size="large"
                >
                  <Clear />
                </IconButton>
                ...
                <DateRange
                  cursor="pointer"
                  htmlColor={
                    !dateTo || rangeIsWrong ? "#FF0000" : "rgba(0, 0, 0, 0.54)"
                  }
                />
              ),
            }}
        ...
        <Grid item xs={12} sm={3} key="service advisor">
          <Autocomplete
            ...
            value={advisor}
            getOptionLabel={(o) =>
              o.fullName ? `${o.fullName} - ${o.dmsId}` : o.dmsId
            }
        ...
        <Grid item xs={12} sm={3} key="scheduler">
          <Autocomplete
            ...
            options={[...schedulerList].sort((a, b) =>
        ...
        <Grid item xs={12} sm={3} key="serviceBook">
          <Autocomplete
            ...
            options={[...serviceBookList].sort((a, b) =>
