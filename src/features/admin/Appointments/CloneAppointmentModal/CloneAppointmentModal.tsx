...
const CloneAppointmentModal: React.FC<
  DialogProps & { onViewClose: TCallback }
> = ({ onViewClose, ...props }) => {
  const { currentAppointment, isAppointmentLoading } = useSelector(
    (state: RootState) => state.appointments,
  );
  const { isAppointmentSaving } = useSelector(
    (state: RootState) => state.appointmentFrame,
  );
  const { appointment, serviceValetAppointment, appointmentSlots } =
  ...
  const handleConfirm = () => {
    selectedSC &&
      dispatch(cloneAppointment(selectedSC.id, onCloneSuccess, showError));
  };
  ...
  const onRedirect = async () => {
    if (newHashKey) {
      const url = window.location.href.replace(
        "/admin/appointments",
        `/appointment-update/${newHashKey}?fromAdmin=true`,
      );
    ...
    <BaseModal {...props} width={900}>
      ...
      <DialogContent style={{ padding: "0 36px 36px 36px" }}>
        <SlotsScreenWrapper>
          ...
          {currentAppointment?.serviceTypeOption?.type ===
          EServiceType.PickUpDropOff ? (
            <SVAppointmentTimeSelector date={date} loading={isLoading} />
          ) : (
            <AppointmentTimeSelector
              appointments={
                groupedAppointments[
                  dayjs(date).toISOString().replace(".000", "")
                ]
              }
              date={date}
              loading={isLoading}
