...
const RecallParts = () => {
  ...
  const [selectedOpsCode, setSelectedOpsCode] =
    useState<IAssignedServiceRequest | null>(null);
  ...
  useEffect(() => {
    if (selectedSC && allAssignedList) {
      const opsCode = allAssignedList.find(
        (item) => item.id === selectedSC.recallServiceRequestId,
      );
