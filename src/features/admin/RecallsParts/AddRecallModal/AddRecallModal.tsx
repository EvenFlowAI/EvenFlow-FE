...
> = ({ editingItem, open, onClose, setEditingItem }) => {
  ...
  useEffect(() => {
    if (open && editingItem) {
      ...
      const models =
        make?.models.filter((el) =>
          editingItem.models.find((item) => item.id === el.id),
        ) ?? [];
      const sr = allAssignedList.find(
        (item) => item.id === editingItem.serviceRequest?.id,
      );
      setForm(() => ({
  ...
  const onYearChange =
    ...
    (e: ChangeEvent<{}>, value: string | null) => {
      setFormIsChecked(false);
      setForm((form) => ({ ...form, [name]: value ?? "" }));
    };
  ...
  const onSRChange = (
    e: ChangeEvent<{}>,
    value: IAssignedServiceRequest | null,
  ) => {
    setFormIsChecked(false);
    ...
    <BaseModal open={open} onClose={onCancel} width={500}>
      ...
      <DialogContent>
        <TextField
          ...
          error={
            formIsChecked && !form.recallCampaignNumber && !form.oemProgram
          }
        ...
        <Autocomplete
          ...
          renderInput={autocompleteRender({
            ...
            error: Boolean(
              form.yearFrom && form.yearTo && form.yearFrom > form.yearTo,
            ),
          })}
        />
        <Autocomplete
