...
> = (props) => {
  ...
  const [isComplimentaryNameEdit, setComplimentaryNameEdit] =
    useState<boolean>(false);
  const [editingOption, setEditingOption] =
    useState<IPackageOptionDetailed | null>(null);
  const {
      ...
      if (packageData?.serviceRequests) {
        const rows = packageData.serviceRequests
          ...
          .map((request) => ({
            ...
            cellData: packageData.options.map(
              (option: IPackageOptionDetailed) => ({
                optionType: option.type,
                isSelected: !!option.serviceRequests.find(
          ...
          .map((request) => ({
            ...
            cellData: packageData.options.map(
              (option: IPackageOptionDetailed) => ({
                optionType: option.type,
                isSelected: Boolean(
                  option.intervalUpsells.find(
                    (r) => r.serviceRequestId === request.id,
                  ),
                ),
              }),
            ),
  ...
  const onComplimentaryClick = useCallback(
    (item: TCellData, requestId: number): void => {
      if (packageData) {
        const option = packageData.options.find(
          (el) => el.type === item.optionType,
        );
        if (option) {
          const updatedOption = {
            ...option,
            complimentaryServices: option.complimentaryServices.includes(
              requestId,
            )
              ? option.complimentaryServices.filter(
        ...
        if (option) {
          const updatedOption = {
            ...
            serviceRequests: option.serviceRequests.find(
              ...
              ? option.serviceRequests.filter(
                  (request) => request.serviceRequestId !== requestId,
                )
              : [
                  ...option.serviceRequests,
                  { serviceRequestId: requestId, isSendToDMS: true },
                ],
          };
          const updatedData = {
            ...packageData,
        ...
        if (option) {
          const updatedOption = {
            ...
            intervalUpsells: option.intervalUpsells.find(
              ...
              ? option.intervalUpsells.filter(
                  (request) => request.serviceRequestId !== requestId,
                )
              : [
                  ...option.intervalUpsells,
                  { serviceRequestId: requestId, isSendToDMS: true },
                ],
          };
          const updatedData = {
    ...
    (value: string, fieldName: string, optionType: string | number) => {
      if (packageData) {
        if (fieldName.toLowerCase().includes("hours") && Number(value) > 100) {
          ...
          let currentOption = packageData.options.find(
  ...
  const sendRequest = useCallback(
    (data: IPackageById) => {
      const revisedData = data.options.map((option) => ({
        ...
        intervalUpsellServiceLaborHours:
          +option.intervalUpsellServiceLaborHours,
      ...
      try {
        ...
        const upsellsAreIncluded = data.options.find(
          (opt) => opt.intervalUpsells.length,
        );
    ...
    [dispatch, currentPackage, showError, packageData],
  );
  ...
  const onOptionNameChange = useCallback(
    (option: IPackageOptionDetailed, name: string): void => {
      setPackageData((prev) => {
        if (prev) {
          const optionToChange = prev.options.find(
              ...
              <React.Fragment>
                ...
                <SummaryRow
                  ...
                  packageHasComplimentary={Boolean(
                    packageData?.intervalUpsells?.length,
                  )}
                  summaryText="Invoiced Labor Hours:"
                ...
                <SummaryRow
                  ...
                  isComplimentary
                  packageHasComplimentary={Boolean(
                    packageData?.complimentaryServices?.length,
                  )}
                  summaryText="Invoiced Labor Hours:"
