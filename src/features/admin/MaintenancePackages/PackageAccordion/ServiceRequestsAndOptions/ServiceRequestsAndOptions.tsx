...
type TServiceRequestsProps = {
  ...
  setEditingOption: React.Dispatch<
    React.SetStateAction<IPackageOptionDetailed | null>
  >;
  data: TRequestRow[];
};

export const ServiceRequestsWithOptions: React.FC<
  ...
  const getCellClass = useCallback(
    (cellIndex: number, rowIndex: number) => {
      if (props.packageData) {
        if (cellIndex === 0) {
          if (props.packageData?.serviceRequests?.length === 1)
            ...
            {props.packageData?.options
              ? [...props.packageData?.options]
                  ...
                  .map((option: IPackageOptionDetailed) => (
                    ...
                    >
                      {props.editingOption &&
                      props.editingOption.type === option.type ? (
                        <Input
                          value={option.name}
