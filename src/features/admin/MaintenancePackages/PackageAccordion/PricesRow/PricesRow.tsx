...
const PricesRow: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<TProps>>
> = ({ packageData, suggestedPrices, setPackageData }) => {
  ...
  const basePrice = useMemo(
    () =>
      currentPackage?.priceTitles?.find(
        (item) => item.type === EPackagePricingType.BasePrice,
      ),
    [currentPackage],
  );
  const withFeePrice = useMemo(
    () =>
      currentPackage?.priceTitles?.find(
  ...
  const [goodOption, betterOption, bestOption] = useMemo(() => {
    const optOne = packageData?.options?.find(
      (el) => el.type === EMaintenanceOptionType.Base,
    );
    const optTwo = packageData?.options?.find(
      (el) => el.type === EMaintenanceOptionType.Value,
    );
    const optThree = packageData?.options?.find(
      (el) => el.type === EMaintenanceOptionType.Preferred,
    );
  ...
  const goodCorePrice = useMemo(() => {
    ...
    if (goodOption) {
      if (currentPackage?.isShowSuggestedPrice) {
        const suggested = suggestedPrices.find(
          (el) => el.optionType === goodOption.type,
        )?.numberValue;
  ...
  const betterCorePrice = useMemo(() => {
    ...
    if (betterOption) {
      if (currentPackage?.isShowSuggestedPrice) {
        const suggested = suggestedPrices.find(
          (el) => el.optionType === betterOption.type,
        )?.numberValue;
        if (suggested) price = +suggested;
      } else {
  ...
  const bestCorePrice = useMemo(() => {
    ...
    if (bestOption) {
      if (currentPackage?.isShowSuggestedPrice) {
        const suggested = suggestedPrices.find(
          (el) => el.optionType === bestOption.type,
        )?.numberValue;
        if (suggested) price = +suggested;
      } else {
  ...
  const goodUpsellPrice = useMemo(() => {
    ...
    if (goodOption && goodOption.intervalUpsellServicePrice)
      price = +goodCorePrice + +goodOption.intervalUpsellServicePrice;
    return price;
  }, [goodOption, goodCorePrice]);

  const betterUpsellPrice = useMemo(() => {
    let price = betterCorePrice;
    if (betterOption && betterOption.intervalUpsellServicePrice)
      price = +betterCorePrice + +betterOption.intervalUpsellServicePrice;
    return price;
  }, [betterOption, betterCorePrice]);

  const bestUpsellPrice = useMemo(() => {
    let price = bestCorePrice;
    if (bestOption && bestOption.intervalUpsellServicePrice)
      price = +bestCorePrice + +bestOption.intervalUpsellServicePrice;
    return price;
  ...
  const onSavePrice = (title: string) => {
    if (currentPackage && title.length) {
      ...
      dispatch(
        updatePriceTitles(currentPackage.id, { title, type }, () =>
          onPriceUpdated(title, type),
        ),
      );
    }
  };
    ...
    <div>
      ...
      <div className={classes.wrapper}>
        <div className={classes.rightPart}>
          ...
          <PriceTitleEditable
            text={withFeePrice?.title}
            onSave={onSavePriceWithFee}
          />
        </div>
