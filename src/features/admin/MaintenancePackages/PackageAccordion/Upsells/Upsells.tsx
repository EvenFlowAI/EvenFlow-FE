...
const Upsells: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<TUpsellTitleProps>>
> = ({
  isUpsellNameEdit,
  packageData,
  setUpsellNameEdit,
  ...
  const upsellTitle = packageData?.segmentTitles?.find(
    (el) => el.type === ESegmentTitle.IntervalUpsell,
  );

  const onIntervalUpsellNameChange = (
    ...
    if (
      ...
      setPackageData((prev) => {
        if (prev) {
          const updated = {
            type: ESegmentTitle.IntervalUpsell,
