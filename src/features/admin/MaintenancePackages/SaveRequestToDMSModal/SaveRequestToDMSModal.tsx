...
> = ({ packageData, setPackageData, onSave, ...props }) => {
  ...
  const [newUpsellRequests, setNewUpsellRequests] = useState<
    TIntervalUpsellForPackage[]
  >([]);
  ...
  const onCheckboxClick = useCallback(
    (option: IPackageOptionDetailed, requestId: number): void => {
      setTemporaryData((prev) => {
        if (prev) {
          const optionToUpdate = prev.options.find(
          ...
          if (optionToUpdate) {
            ...
            if (request) {
              const updatedRequest = {
                ...request,
                isSendToDMS: !request.isSendToDMS,
              };
              const updatedOption = {
                ...optionToUpdate,
  ...
  return (
    <BaseModal {...props} style={{ minWidth: 1000 }}>
      <DialogTitle onClose={props.onClose}>
        Choose Op Codes to send to DMS
      </DialogTitle>
      <DialogContent>
                ...
                {newRequests
                  ...
                  .map((request, rowIndex) => {
                    return (
                      <TableRow
                        className={
                          rowIndex % 2 === 0 ? classes.row : classes.rowGrey
                        }
                        key={request.code}
                      >
                        <TableCell
                        ...
                        {temporaryData?.options
                          ...
                          .map((option, cellIndex) => {
                            const requestInOption = option.serviceRequests.find(
                              (req) => req.serviceRequestId === request.id,
                            );
                            ...
                            return (
                              ...
                              >
                                <IconButton
                                  onClick={() =>
                                    onCheckboxClick(option, request.id)
                                  }
                                  size="large"
                                >
                                  {requestInOption ? (
                    ...
                    return (
                      ...
                      >
                        ...
                        {temporaryData?.options
                          ...
                          .sort((a, b) => a.type - b.type)
                          .map((option, cellIndex) => {
                            const requestInOption = option.intervalUpsells.find(
                              (req) => req.serviceRequestId === request.id,
                            );
                            ...
                            return (
                              ...
                              >
                                <IconButton
                                  onClick={() =>
                                    onUpsellCheckboxClick(option, request.id)
                                  }
                                  size="large"
                                >
                                  {requestInOption ? (
