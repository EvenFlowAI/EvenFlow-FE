...
const Mileage: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<TMileageProps>>
> = ({ disabled, selectedMileages, setSelectedMileages, setFormIsChecked }) => {
  const { mileage } = useSelector((state: RootState) => state.vehicleDetails);
  const { classes } = useAutocompleteStyles();

  const sortMileage = (a: string, b: string) => {
    return selectedMileages.includes(a)
      ? selectedMileages.includes(b)
        ? 0
        : -1
      : 1;
  };
  ...
  const renderOption = useCallback(
    (props: any, option: string) => {
      const allMileagesSelected = mileage.length
        ? mileage.every((item) =>
            selectedMileages.includes(item.value.toString()),
          )
        : false;
