...
> = ({ viewMode, ...props }) => {
  ...
  useEffect(() => {
    if (selectedSC) {
      Api.call<IServiceCenterExtended>(Api.endpoints.ServiceCenters.Retrieve, {
  ...
  const handleSave = async () => {
    if (!selectedSC) {
      ...
      setSave(true);
      try {
        await Api.call(Api.endpoints.ServiceCenters.UpdateAddress, {
          data: form,
          urlParams: { id: selectedSC.id },
        });
  ...
  return (
    <BaseModal {...props} maxWidth="sm">
      <DialogTitle onClose={props.onClose}>{viewMode ? 'View' : 'Edit'} Address</DialogTitle>
      <DialogContent>
        <EditForm
          viewMode={viewMode}
