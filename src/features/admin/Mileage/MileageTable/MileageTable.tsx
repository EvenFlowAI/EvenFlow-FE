...
const RowData = [
  {
    val: (el: IMileage) => `${el.value}`,
    header: "Estimated Mileage",
    orderId: "value",
  },
];
...
export const MileageTable = () => {
  ...
  const openMenu =
    (el: IMileage) => (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
  ...
  const handleRemove = async () => {
    if (currentMileage && selectedSC) {
      dispatch(
        removeMileage(currentMileage.id, selectedSC.id, onRemoveSuccess, (e) =>
          showError(e),
        ),
      );
    }
