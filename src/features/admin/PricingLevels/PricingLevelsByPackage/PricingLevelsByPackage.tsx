...
const RowData: TableRowDataType<TPackagePricingLevel>[] = [
  {
    val: (el: TPackagePricingLevel, index: number) => `${index + 1}`,
    header: "#",
  },
  {
    val: (el: TPackagePricingLevel) => el.maintenancePackageName,
    header: "Package Name",
    width: "25%",
  },
  {
    val: (el: TPackagePricingLevel) => `${el.maintenancePackageId}`,
    header: "ID",
  },
  {
    val: (el: TPackagePricingLevel) => el.maintenancePackageOptionName,
    header: "Package Option Name",
    width: "20%",
  },
  {
    val: (el: TPackagePricingLevel) =>
      el.discount ? `${el.discount}%` : "Default",
    header: "Discount",
  },
  {
    val: (el: TPackagePricingLevel) =>
      el.premium ? `${el.premium}%` : "Default",
    header: "Premium",
  },
];

const PricingLevelsByPackage = () => {
  const { mpPricingLevels, isLoading, mpOptionsList } = useSelector(
    (state: RootState) => state.pricingSettings,
  );
  const [editElement, setEditElement] = useState<TPackagePricingLevel | null>(
    null,
  );
    ...
    if (mpOptionsList && mpPricingLevels) {
      setData(() => {
        return mpOptionsList.map((item) => {
          const levelsItem = mpPricingLevels.find(
            (el) =>
              el.maintenancePackageOptionId === item.maintenancePackageOptionId,
          );
          let discount = null;
          let premium = null;
