...
const EditPackagePricingLevelModal: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<TEditPricingLevelsProps>>
> = ({ prisingLevel, ...props }) => {
  ...
  const onSave = useCallback(() => {
    ...
    if (prisingLevel && selectedSC) {
      ...
      try {
        dispatch(
          updateMPPricingLevels(
            prisingLevel.maintenancePackageOptionId,
            data,
            onCancel,
          ),
        );
  ...
  return (
    <BaseModal {...props} width={540} onClose={onCancel}>
      <DialogTitle onClose={onCancel}>
        Edit Pricing Levels by Maintenance Package
      </DialogTitle>
      <DialogContent>
        <TextField
          fullWidth
...
export default EditPackagePricingLevelModal;
