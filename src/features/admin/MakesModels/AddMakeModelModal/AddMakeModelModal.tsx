...
> = ({ isEditing, onClose, ...props }) => {
  const { currentMake, makes } = useSelector(
    (state: RootState) => state.vehicleDetails,
  );
  const { selectedSC } = useSelector(
  ...
  const onSave = () => {
    ...
    const modelsList: string[] = newModel?.length
      ? [...models, newModel]
      : models;
    if (make && modelsList.length) {
      if (isEditing && currentMake?.id) {
        ...
        if (selectedSC) {
          if (
            makes.find((item) => item.name.toUpperCase() === make.toUpperCase())
          ) {
  ...
  return (
    <BaseModal {...props} width={540} onClose={onCancel}>
      <DialogTitle onClose={onCancel}>
        {isEditing ? "Edit" : "Add"} Make and Model
      </DialogTitle>
      <DialogContent>
        <TextField
          fullWidth
          ...
          error={
            ...
            (!make ||
              !!makes.find(
                (item) => item.name.toUpperCase() === make.toUpperCase(),
              ))
          }
        ...
        {Boolean(models.length) && (
          <div className={classes.modelsWrapper}>
            {models.map((model) => (
              <Chip key={model} name={model} onDelete={onModelDelete} />
            ))}
          </div>
        )}
