...
const TimeRangesAndCapacity = () => {
  ...
  const [currentRange, setCurrentRange] =
    useState<ITimeRangeAndCapacity | null>(null);
    ...
    if (selectedSC) {
      setTableData(() => {
        return [1, 2, 3, 4, 5, 6, 0].map((day) => {
          const timeRange = timeRangesAndCapacity.find(
          ...
          return {
            ...
            pickUpMin: timeRange?.pickUpMin
              ? dayjs(timeRange?.pickUpMin, timeWithSecond).format(
                  time24HourFormat,
                )
              : "-",
            pickUpMax: timeRange?.pickUpMax
              ? dayjs(timeRange?.pickUpMax, timeWithSecond).format(
                  time24HourFormat,
                )
              : "-",
            dropOffMin: timeRange?.dropOffMin
              ? dayjs(timeRange?.dropOffMin, timeWithSecond).format(
                  time24HourFormat,
                )
              : "-",
            dropOffMax: timeRange?.dropOffMax
              ? dayjs(timeRange?.dropOffMax, timeWithSecond).format(
                  time24HourFormat,
                )
              : "-",
  ...
  const RowData: TableRowDataType<ITimeRangeAndCapacity>[] = [
    {
      header: "Day Of Week",
      val: (el) =>
        typeof el.dayOfWeek !== "undefined"
          ? dayjs().set("day", el.dayOfWeek).format("dddd")
          : "",
    },
    {
      header: "Pick Up Min",
