...
const IntervalUpsellModal: React.FC<
  React.PropsWithChildren<
    React.PropsWithChildren<DialogProps<IUpsellServiceRequest>>
  >
> = ({ payload, ...props }) => {
  ...
  useEffect(() => {
    if (props.open && payload) {
      setForm({
        ...initialForm,
        description:
          payload?.description || payload?.serviceRequest?.description || "",
        durationInHours:
        ...
        try {
          const data: IUpsellServiceRequestUpdate = {
            ...
            durationInHours: form.durationInHours
              ? Number(form.durationInHours)
              : null,
            invoiceAmount: form.invoiceAmount
              ? Number(form.invoiceAmount)
              : null,
            partsUnitCost: form.partsUnitCost
              ? Number(form.partsUnitCost)
              : null,
            numberOfParts: form.numberOfParts
              ? Number(form.numberOfParts)
              : null,
          };
          dispatch(
