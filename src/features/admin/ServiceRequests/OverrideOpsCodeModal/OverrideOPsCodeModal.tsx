...
export const OverrideOPsCodeModal: React.FC<
  React.PropsWithChildren<
    React.PropsWithChildren<DialogProps<IAssignedServiceRequest>>
  >
  ...
  useEffect(() => {
    if (props.open && payload?.serviceRequestOverride) {
      ...
      setForm({
        ...
        warrantyInvoiceAmount:
          override?.warrantyInvoiceAmount?.toFixed(2) || "",
        skillLevelOfTechnicians: override?.skillLevelOfTechnicians || 0,
        partsUnitCost:
          override?.partsUnitCost?.toFixed(2) ??
          request?.partsUnitCost?.toFixed(2) ??
          "",
        numberOfParts:
          override?.numberOfParts?.toString() ??
          request?.numberOfParts?.toString() ??
          "",
      });
    }
  }, [payload, props.open]);
        ...
        const data: IServiceRequestOverrideEditRequest = {
          serviceRequestInfo: {
            ...
            ...Object.entries(f).reduce(
              (acc, e) => ({ ...acc, [e[0]]: e[1] ? Number(e[1]) : undefined }),
        ...
        <Grid container spacing={3} alignItems="flex-end">
          ...
          <Grid item xs={6}>
            <TextField
              ...
              placeholder={
                payload ? String(payload.serviceRequest.durationInHours) : ""
              }
              onChange={handleChange}
              type="number"
            ...
            <Select
              ...
              input={
                <TextField
                  ...
                  placeholder="Technician Level"
                />
              }
              style={{
                color: form.skillLevelOfTechnicians ? "inherit" : "#858585",
              }}
              onChange={handleLevelChange}
            >
              <EmptyMenuItem value={0}>Technician Level</EmptyMenuItem>
              {levels.map((level) => (
                <MenuItem value={level.value} key={level.name}>
                  {level.name}
                </MenuItem>
              ))}
            </Select>
          ...
          <Grid item xs={6}>
            <TextField
              ...
              placeholder={
                payload
                  ? String(payload.serviceRequest.warrantyInvoiceAmount)
                  : ""
              }
              onChange={handleChange}
              type="number"
          ...
          <Grid item xs={6}>
            <TextField
              ...
              placeholder={
                payload ? String(payload.serviceRequest.invoiceAmount) : ""
              }
              onChange={handleChange}
              type="number"
              inputProps={{ min: 1, step: 0.01 }}
          ...
          <Grid item xs={6}>
            <TextField
              ...
              placeholder={
                payload?.serviceRequestOverride?.partsUnitCost?.toString() ||
                payload?.serviceRequest?.partsUnitCost?.toString() ||
                ""
              }
              onChange={handleChange}
              type="number"
          ...
          <Grid item xs={6}>
            <TextField
              ...
              placeholder={
                payload?.serviceRequestOverride?.numberOfParts?.toString() ||
