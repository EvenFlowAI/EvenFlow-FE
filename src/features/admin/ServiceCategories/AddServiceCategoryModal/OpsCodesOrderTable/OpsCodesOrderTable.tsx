...
> = ({
  ...
  const checkError = (el: IAssignedServiceRequest): boolean => {
    const codes = selectedCodes.filter((item) =>
      wrongOrderIndexes.includes(+item.orderIndex),
    );
  ...
  const RowData: TableRowDataType<IAssignedServiceRequest>[] = [
    {
      ...
      val: (el) => (
        <TextField
          ...
          value={
            selectedCodes.find((item) => item.id === el.id)?.orderIndex ?? ""
          }
      ...
      if (!disabled) {
        setSelectedCodes((prev) => {
          ...
          if (codeToChange) {
            ...
            const indexes = prev.map((el) =>
              el.orderIndex ? +el.orderIndex : 0,
            );
            const orderIndex = indexes.length
              ? String(Math.max(...indexes) + 1)
              : "0";
