...
const RowData: TableRowDataType<ICategory>[] = [
  {
    val: (el: ICategory) => el.name,
    header: "Service Category Name",
    width: 300,
  },
  {
    val: (el: ICategory) => el.orderIndex?.toString() ?? "",
    header: "Order Index",
    align: "center",
    width: 150,
  },
...
export const ServiceCategoriesTable: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<{ tabValue: string }>>
> = ({ tabValue }) => {
  ...
  const openMenu =
    (el: ICategory) => (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
      setCurrentItem(el);
  ...
  const handleRemove = async () => {
    if (!currentItem) {
      ...
      try {
        ...
        if (currentItem.id)
          dispatch(deleteCategoryById(currentItem.id, tabServiceType));
