...
> = ({ editingElement, ...props }) => {
    ...
    if (editingElement && props.open) {
      ...
      if (rules) {
        ...
        const [startHours, startMinutes, startSeconds] =
          rules.timeOfDay.start.split(":");
        const [endHours, endMinutes, endSeconds] =
          rules.timeOfDay.end.split(":");
  ...
  const onDayOfWeekChange = useCallback(
    (e: ChangeEvent<{}>, value: TOption[]) => {
      setFormIsChecked(false);
      if (
        value.find((option) => option.value === ETransportationDays.EveryDay)
      ) {
        setDaysOfWeek(
  ...
  const renderRequestOption = useCallback(
    (props: HTMLAttributes<HTMLLIElement>, option: TOption) => {
      const checked =
        !!serviceRequests.find((item) => item.value === option.value) ||
        allRequestsSelected;
      return (
        <li
          style={{ display: "flex", alignItems: "center" }}
  ...
  const onSave = useCallback(() => {
    ...
    if (selectedSC && editingElement && isValid()) {
      ...
      if (
        daysOfWeek.length &&
        daysOfWeek.length === dayOFWeekOptions.length - 1
      ) {
        ...
        <div className={classes.wrapper}>
          <Autocomplete
            ...
            isOptionEqualToValue={(option, value) =>
              option.name === ECustomerSegment[+value]
            }
