...
const AncillaryPriceByDistance: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<TByDistanceProps>>
> = ({ data, onItemDelete, onItemSave, onAddRange, isLoading }) => {
  ...
  >(null);
  const [editedItem, setEditedItem] = useState<IDistancePriceSettings | null>(
    null,
  );
  const [nextEditedItem, setNextEditedItem] =
    useState<IDistancePriceSettings | null>(null);
  ...
  const checkIsValid = () => {
    if (editedItem) {
      ...
      if (
        updated &&
        updated.costPerMile === 0 &&
        updated.serviceMultiplier === 0
      ) {
        showError(
          "Service Multiplier' or 'Cost Per Mile' must be greater than 0",
        );
        return false;
      }
      if (updated && updated.costPerMile > 0 && updated.serviceMultiplier > 0) {
        showError(
          "Only one value can be greater than 0: 'Service Multiplier' or 'Cost Per Mile'",
        );
        return false;
      }
    }
      ...
      if (editedItem && Number(value) >= 0) {
        setDistanceData((prev) => {
          ...
          const nextItem = prev.find(
            (el) => el.orderIndex === editedItem.orderIndex + 1,
          );
          ...
          if (itemToUpdate) {
            ...
            if (nextItem && fieldName === "maxValue") {
              if (newValue > nextItem.maxValue) {
                showError(
                  '"Max Value" of the Distance Range must be less than or equal to "Max Value" of the next Distance Range',
                );
                return prev;
              }
            ...
            const filtered = nextUpdated
              ? prev.filter(
                  (el) => el.id !== editedItem?.id && el.id !== nextUpdated?.id,
                )
              : prev.filter((el) => el.id !== editedItem?.id);
            const data = nextUpdated
              ? [...filtered, updated, nextUpdated]
              : [...filtered, updated];
          ...
          <TableHead>
            <TableRow>
              ...
              <HeaderTableCell align="left" size="small" width={190}>
                <div className="distanceCell">Cost Per Mile ($)</div>
              </HeaderTableCell>
              ...
              <LastTableCell align="left" size="small" width={130}>
                {isEdit ? (
                  <>
                    ...
                    <Button onClick={onSave} color="primary" size="small">
                      Save
                    </Button>
                  </>
                ) : null}
          ...
          <TableBody>
            {distanceData.map((item, index) => (
              <TableRow key={item.id}>
                <FirstCell size="small" align="center">
                  {index + 1}.
                </FirstCell>
