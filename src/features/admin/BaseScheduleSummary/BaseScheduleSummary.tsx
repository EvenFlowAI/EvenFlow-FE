...
const BaseScheduleByEmployee = () => {
  const { baseSummary, loading } = useSelector(
  ...
  useEffect(() => {
    if (selectedSC?.id) {
      dispatch(
        loadBaseSummary(selectedSC?.id, order.orderBy, order.isAscending),
      );
    }
  }, [selectedSC, order]);
  ...
  const getDailyHoursFromMonday = (item: IRoleHours) => {
    ...
    return [...hours, item.dailyHours[0]].map((day) => {
      return (
        <StyledScheduleCell key={day.day}>
          {day.value.toFixed(1)}
        </StyledScheduleCell>
      );
    });
  ...
  const getTotalHoursFromMonday = () => {
    if (baseSummary?.totalHours?.length) {
      const hours = baseSummary.totalHours.slice(
        1,
        baseSummary.totalHours.length,
      );
      return [...hours, baseSummary.totalHours[0]].map((item) => {
        return (
          <StyledScheduleCell key={item.day}>
          ...
          {baseSummary ? (
            <TableBody>
              ...
              <ScheduleTableFooterRow key="total">
                <StyledScheduleCell />
                <ScheduleTableTotalCell key="totalCell">
                  Total
                </ScheduleTableTotalCell>
