...
import { ShowEditDataCard } from "../../../components/wrappers/ShowEditDataCard/ShowEditDataCard";
...
export const windowsContent: TWindowContent = {
  [ESearchWindowType.FirstAvailable]: {
    helperText:
      "Set the optimization window for available time slots when first available date search is entered",
    label: "Days",
  ...
  [ESearchWindowType.SpecificDate]: {
    prefix: "+/- ",
    helperText:
      "Set the optimization window for available time slots when a specific date search is entered",
    label: "Days",
...
const SearchWindows = () => {
  const optParams = useSelector(
    (state: RootState) => state.optimizationWindows.dataList,
  );
  ...
  const {
    ...
    onOpen: onSpecificOpen,
    onClose: onSpecificClose,
  } = useModal();
  const firstAvailable = useMemo(() => {
    return (
      optParams.find(
        (el) => el.type === EOptimizationWindowType.FirstAvailable,
      ) ?? {
  ...
  const specificDate = useMemo(() => {
    return (
      optParams.find(
        (el) => el.type === EOptimizationWindowType.SpecificDate,
      ) ?? {
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} sm={6} md={4} key={"First Available Search"}>
          <ShowEditDataCard
            ...
            prefix={
              windowsContent[EOptimizationWindowType.FirstAvailable].prefix
            }
            suffix={
              windowsContent[EOptimizationWindowType.FirstAvailable].suffix
            }
            helperText={
              windowsContent[EOptimizationWindowType.FirstAvailable].helperText
            }
          />
        </Grid>
        <Grid item xs={12} sm={6} md={4} key={"Specific Date Search"}>
          <ShowEditDataCard
            ...
            helperText={
              windowsContent[EOptimizationWindowType.SpecificDate].helperText
            }
          />
        </Grid>
      </Grid>
