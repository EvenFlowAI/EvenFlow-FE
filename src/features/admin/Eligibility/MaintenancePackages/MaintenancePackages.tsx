...
const MaintenancePackages = () => {
  const { isLoading, mpList } = useSelector(
  ...
  const handlePricingDisplayType =
    (id: number) => (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.value && selectedSC) {
        dispatch(
          changeMPPrisingDisplayType(id, +e.target.value, selectedSC.id, (e) =>
            showError(e),
          ),
        );
      }
    ...
    <div>
      ...
      <Box m={2} mt={1}>
        <Caption
          title={
            <span>
              If Pricing Optimization is turned off, then price displayed is
              tied to
              <span
                style={{
                  color: "#7898FF",
