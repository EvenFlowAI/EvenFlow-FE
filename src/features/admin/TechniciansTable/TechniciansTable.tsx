...
const rowData: TableRowDataTypeResp<IEmployee>[] = [
  ...
  {
    header: "Level",
    val: (v) => v.employeeInfo?.skillLevel.toString() || "-",
    align: "center",
  },
  {
    header: "Hourly Rate",
    val: (v) => `$${v.employeeInfo?.hourlyRate || 0}`,
    align: "right",
  },
  {
    header: "Overtime Rate",
    val: (v) => `$${v.employeeInfo?.overtimeRate || 0}`,
    align: "right",
    xsHidden: true,
  },
];
...
export const TechniciansTable = () => {
  ...
  const openMenu =
    (u: IEmployee) => (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
      setEdit({
        ...u,
