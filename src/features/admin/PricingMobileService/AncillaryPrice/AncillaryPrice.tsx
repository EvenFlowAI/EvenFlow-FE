...
const AncillaryPrice = () => {
  ...
  const [typeOfPrice, setTypeOfPrice] = useState<EAncillaryPriceType>(
  ...
  const onDelete = (itemId: number) => {
    if (selectedSC)
      dispatch(
        deleteMobileServicePrisingByDistance(selectedSC.id, itemId, onError),
      );
  };
  ...
  const onAddRange = (data: TDistanceRange) => {
    if (selectedSC) {
      ...
      dispatch(
        addMobileServiceDistanceRange(selectedSC.id, data, onSuccess, onError),
      );
    }
  };
  ...
  const onSaveZonePricing = (data: IZonePriceSettings) => {
    if (selectedSC)
      dispatch(
        updateMobileServicePrisingByZones(selectedSC.id, data.id, data, (err) =>
          showError(err),
        ),
      );
  };
      ...
      askConfirm({
        ...
        onConfirm: () => {
          try {
            ...
            dispatch(
              changeMobileServicePriceSettings(selectedSC.id, value, (err) =>
                showError(err),
              ),
            );
          } catch (e) {
    ...
    <TablesWrapper>
      {isPricingByZoneLoading ? (
        ...
        <div className={classes.wrapper}>
          <div className={classes.optionsTitleWrapper}>
            Convenience Fees Based On:{" "}
          </div>
          <RadioGroup
            row
            ...
            name="countType"
            value={
              typeOfPrice === EAncillaryPriceType.Zone ? "byZone" : "byDistance"
            }
            onChange={onChange}
          >
            <FormControlLabel
