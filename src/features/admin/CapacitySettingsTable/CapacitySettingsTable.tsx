...
const CapacitySettingsTable = () => {
  const { capacitySettings } = useSelector(
  ...
  useEffect(() => {
    if (selectedSC) {
      dispatch(
        loadCapacitySettings(selectedSC.id, dayjs().format("dddd"), showError),
      );
    ...
    <div>
      ...
      <Menu open={Boolean(anchorEl)} anchorEl={anchorEl} onClose={closeMenu}>
        <MenuItem
          ...
          disabled={!editedItem?.serviceBookId}
        >
          Configure
        </MenuItem>
        <MenuItem onClick={handleEdit}>Edit</MenuItem>
      </Menu>
      <ServiceBookSettingsModal
        open={isEditOpen}
        onClose={onEditClose}
