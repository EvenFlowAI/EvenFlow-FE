...
> = ({ editingItem, ...props }) => {
  ...
  const [firstScreenOptionName, setFirstScreenOptionName] =
    useState<string>("");
  ...
  const [selectedServiceType, setSelectedServiceType] =
    useState<TOption | null>({ value: "0", name: "Visit Center" });
  ...
  const enabledTransportationOptions = useMemo(
    () => options.filter((op) => op.state),
    [options],
  );
  ...
  useEffect(() => {
    if (props.open && editingItem) {
      ...
      if (editingItem.transportationOption?.id) {
        const transportation = options.find(
          (item) => item.id === editingItem.transportationOption?.id,
        );
        transportation && setDefaultTransportation(transportation);
      }
      if (editingItem.type >= 0) {
        const serviceTypeOption = getServiceTypeOptions().find(
      ...
      if (editingItem.taglineFontColorHex)
        setTaglineColor(editingItem.taglineFontColorHex);
    }
  ...
  const getServiceTypeOptions = () => {
    return Object.entries(serviceTypeNames).map(([value, name]) => ({
      value,
      name,
    }));
  };
  ...
  const onServiceTypeChange = useCallback(
    (e: React.ChangeEvent<{}>, value: TOption | null) => {
      setFormIsChecked(false);
        ...
        <div className={classes.inputsWrapper}>
          ...
          <Autocomplete
            ...
            isOptionEqualToValue={(option) =>
              option.value === selectedServiceType?.value
            }
          ...
          <FileInput
            ...
            label={`${fileState.file || editingItem?.iconPath ? "Update" : "Upload"} Option Icon`}
          />
          <Autocomplete
            disableClearable
          ...
          <Autocomplete
            ...
            isOptionEqualToValue={(option) =>
              option.id === defaultTransportation?.id
            }
            getOptionLabel={(o) =>
              getTransportationOptionString(o.type.toString())
            }
          ...
          <div>
            <TextField
              ...
              disabled={
                selectedServiceType?.value !== EServiceType.General.toString()
              }
