...
const CapacityTechnicians = () => {
  const { capacityTypeOption, isSettingLoading } = useSelector(
  ...
  useEffect(() => {
    ...
    dispatch(
      setDateRange({
        from: dateFrom,
        to: dayjs(dateFrom, CALENDAR_FORMAT)
          .add(1, "week")
          .subtract(1, "day")
    ...
    if (selectedSC) {
      dispatch(
        updateTechniciansCapacityType(
          selectedSC.id,
          value === "DailyVehicles"
            ? ECapacityType.DailyVehicles
            : ECapacityType.AvailableBillHours,
          selectedTab === "0"
            ? ECapacityType.DailyVehicles
            : ECapacityType.AvailableBillHours,
          showError,
    ...
    <div>
      {isSettingLoading ? (
        ...
        <RadioBlock>
          ...
          <RadioGroup
            ...
            name="countType"
            value={
              capacityTypeOption === ECapacityType.DailyVehicles
                ? "DailyVehicles"
                : "AvailableBillHours"
            }
            onChange={handleType}
      ...
      <TabContext value={selectedTab}>
        ...
        >
          ...
          <Tab
            label="Available Bill Hour Capacity"
            value="1"
            key="Available Bill Hour Capacity"
          />
        </TabList>
