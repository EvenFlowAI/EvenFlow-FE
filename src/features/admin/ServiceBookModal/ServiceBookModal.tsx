...
export const ServiceBookModal: React.FC<
  DialogProps & { editingItemId: number | undefined }
> = ({ onAction, editingItemId, ...props }) => {
  ...
  const { options: transportations, isLoading: isTransportationLoading } =
    useSelector(({ transportation }: RootState) => transportation);
  const { zones: serviceValetZones } = useSelector(
  ...
  const [selectedServiceValetZones, setSelectedServiceValetZones] = useState<
    TZone[]
  >([]);
  ...
  const appointmentTypeOptions: TOption[] = useMemo(
    () =>
      getOptions(
        Object.keys(EAppointmentType).filter((key) => Number.isNaN(+key)),
      ),
    [],
  );
  ...
  useEffect(() => {
    if (props.open) {
      ...
      if (typeof podById?.jobType !== "undefined") {
        const selectedJobType = jobTypeOptions.find(
          (item) => item.value === podById.jobType,
        );
      ...
      if (podById?.mobileZones) {
        setMobileZones(
          zones.filter((zone) =>
            podById?.mobileZones?.find((item) => item.id === zone.id),
          ),
        );
      } else {
        setMobileZones([]);
      ...
      if (podById?.engineTypes) {
        setSelectedEngineTypes(
          engineTypes.filter((zone) =>
            podById?.engineTypes?.find((item) => item.id === zone.id),
          ),
        );
      } else {
        setSelectedEngineTypes([]);
  ...
  useEffect(() => {
    ...
    makesModels.forEach((item) => {
      const makeIsSelected = podById?.vehicleMakes?.find(
    ...
    if (podById?.vehicleModels?.length) {
      ...
      const filteredModels = podById?.vehicleModels?.filter((item) =>
        modelsIDs.includes(item.id),
      );
      ...
      if (checkIsValid()) {
        ...
        try {
          ...
          if (transportationOptions?.length)
            data.transportationOptionIds = transportationOptions.map(
              (el) => el.id,
            );
          if (mileageFrom) {
  ...
  const onAppointmentTypeChange = useCallback(
    (e: ChangeEvent<{}>, value: TOption | null) => {
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                value={mileageFrom}
                error={
                  mileageFrom ? formIsChecked && mileageFromIsInvalid : false
                }
                disabled={podsLoading || loading}
              />
            </Grid>
            ...
            <Grid item xs={12} sm={12} md={6}>
              <Autocomplete
                ...
                getOptionLabel={(i) =>
                  getTransportationOptionString(i.type.toString())
                }
                isOptionEqualToValue={(o, v) => o.id === v.id}
                renderOption={autocompleteOptionsRender((e) =>
                  getTransportationOptionString(e.type),
                )}
