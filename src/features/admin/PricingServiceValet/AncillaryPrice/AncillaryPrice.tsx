...
const AncillaryPrice = () => {
  ...
  const onDeleteDistanceRange = (itemId: number) => {
    if (selectedSC)
      dispatch(
        deleteServiceValetPrisingByDistance(selectedSC.id, itemId, onError),
      );
  };
  ...
  const onAddRange = (data: TDistanceRange) => {
    if (selectedSC) {
      ...
      dispatch(
        addServiceValetDistanceRange(selectedSC.id, data, onSuccess, onError),
      );
    }
  };
  ...
  const onSaveZonePricing = (data: IZonePriceSettings) => {
    if (selectedSC)
      dispatch(
        updateServiceValetPrisingByZones(selectedSC.id, data.id, data, (err) =>
          showError(err),
        ),
      );
  };
      ...
      askConfirm({
        ...
        onConfirm: () => {
          try {
            ...
            dispatch(
              changeServiceValetPriceSettings(selectedSC.id, value, (err) =>
                showError(err),
              ),
            );
          } catch (e) {
