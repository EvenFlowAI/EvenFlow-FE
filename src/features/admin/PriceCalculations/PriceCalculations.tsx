...
import {
  PaperTitle,
  TableContainer,
} from "../../../pages/admin/ServicePricingSettings/UI";
import {
  Box,
  Divider,
  TableBody,
...
export const PriceCalculations = () => {
  ...
  const [demand, setDemand] = useState<TEnumMap<EDemandCategory> | null>({
    id: EDemandCategory.Average,
    label: "Average",
  });
  const dispatch = useDispatch();
  const data = useSelector(mappedCalculationsSelector);
  const srList = useSelector(
    (state: RootState) => state.serviceRequests.scRequestsShort,
  );
  ...
  const handleDemandChange = (
    e: any,
    value: TEnumMap<EDemandCategory> | null,
  ) => {
    setDemand(value);
  };
