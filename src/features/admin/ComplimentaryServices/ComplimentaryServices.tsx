...
export const ComplimentaryServices = () => {
  ...
  const [editedItem, setEditedItem] = useState<
    IComplimentaryServiceByQuery | undefined
  >(undefined);
  ...
  const {
    ...
    onOpen: onAddManuallyOpen,
    onClose: onAddManuallyClose,
  } = useModal();
  const {
  ...
  const tableData: TableRowDataType<IComplimentaryServiceByQuery>[] = [
    ...
    {
      header: "Labor Hours",
      val: (el) => `${el.durationInHours}`,
      width: 150,
      orderId: "durationInHours",
    },
    {
      header: "Market Rate",
  ...
  useEffect(() => {
    setSelectedOpsCodes(() => {
      const data: number[] = [];
      allComplimentary.forEach(
        (item) => item.serviceRequestId && data.push(item.serviceRequestId),
      );
      return data;
    });
  }, [allComplimentary]);
  ...
  const handleRemove = async () => {
    if (selectedSC && editedItem) {
      try {
        await Api.call(Api.endpoints.ComplimentaryServices.Remove, {
  ...
  const onAddOpsCode = async (
    selectedCodes: number[],
    serviceCenterId: number,
  ) => {
    try {
      const newCodes = selectedCodes.filter(
        (item) => !selectedOpsCodes.includes(item),
      );
      await dispatch(
        addOpsCodeFromList(
          newCodes,
          ...
          () =>
            showMessage(
              `${newCodes.length} ${newCodes.length > 1 ? "Codes" : "Code"} added`,
            ),
        ),
      );
    } catch (e) {
  ...
  return (
    <div>
      ...
      <AddServiceManually
        ...
        title={
          editedItem ? "Edit Complimentary Service" : "Add Service Manually"
        }
