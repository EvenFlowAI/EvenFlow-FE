...
const rowData: TableRowDataType<IDemandSegment>[] = [
  { header: "Demand Segments", val: (el, idx) => String(idx + 1) },
...
> = ({ onAction, payload, ...props }) => {
  ...
  const actions = (el: IDemandSegment) => {
    return (
      <Button onClick={askRemove(el)} variant="outlined" color="primary">
        Delete
      </Button>
    );
  };
  const askRemove = (el: IDemandSegment) => () => {
    const segmentIndex = demandSegmentList.findIndex(
      (item) => item.id === el.id,
    );
    askConfirm({
  ...
  const handleRemoveSegment = (el: IDemandSegment) => async () => {
    try {
      await Api.call(Api.endpoints.AppointmentAllocation.RemoveDemandSegment, {
        urlParams: { id: el.id },
      });
