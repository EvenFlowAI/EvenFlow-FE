...
import {
  loadBaseEmployeeSchedule,
  updateBaseEmployeeSchedule,
...
const EmployeeTimeScheduleSetUp: React.FC<TProps> = ({
  open,
  onClose,
  editingItem,
  getData,
}) => {
  const { employeeSchedule, loading } = useSelector(
    ...
    if (employeeSchedule) {
      setCurrentSchedule(
        daysList.map((dayNumber) => {
          const existingSchedule = employeeSchedule.dayOfWeekSchedules.find(
            (el) => el.dayOfWeek === dayNumber,
          );
          const schedule = hoursOfOperations.find(
            (el) => el.dayOfWeek === dayNumber,
          );
  ...
  const checkIsValid = (): boolean => {
    return currentSchedule
      ...
      .every((item) => {
        const schedule = hoursOfOperations.find(
          (el) => el.dayOfWeek === item.dayOfWeek,
        );
        return (
          dayjs(item.to, timeSpanString).isAfter(
            dayjs(item.from, timeSpanString),
            "minute",
          ) &&
          dayjs(item.to, timeSpanString).isSameOrBefore(
            dayjs(schedule?.to, timeSpanString),
            "minute",
          ) &&
          dayjs(item.from, timeSpanString).isSameOrAfter(
    ...
    if (selectedSC && editingItem) {
      ...
      if (checkIsValid()) {
        const data: TSetScheduleData = {
          ...
          dayOfWeekSchedules: currentSchedule.filter(
            (item) => item.isOnSchedule,
          ),
        };
        dispatch(updateBaseEmployeeSchedule(data, onSuccess, showError));
      } else {
        showError(
          '"To" must be later then "From" and inside of the Hours Of Operations',
        );
      }
    }
  };
      ...
      <DialogContent>
        {loading || employeesLoading ? (
          ...
          <>
            ...
            {daysList.map((day) => {
              const scheduleItem = currentSchedule.find(
                (el) => el.dayOfWeek === day,
              );
              const schedule = hoursOfOperations.find(
                (el) => el.dayOfWeek === day,
