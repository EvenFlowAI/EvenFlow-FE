...
const ShowDropOffTimeModal: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<DialogProps>>
> = ({ onClose, open }) => {
  const { centerSettings } = useSelector(
  ...
  const onSave = () => {
    if (selectedSC) {
      ...
      if (!isShowTime) {
        if (text.trim().length) {
          dispatch(
            updateShowDropOffTime(selectedSC.id, data, onCancel, showError),
          );
        } else {
  ...
  return (
    <BaseModal onClose={onCancel} open={open} width={375}>
      ...
      <DialogContent>
        ...
        <p>
          Enter the text to display in the booking flow about communicating drop
          off expectations
        </p>
        <Textarea
          disabled={isShowTime}
        ...
        />
        <Warning>
          {!isShowTime && !text.trim().length ? "Please enter a text" : ""}
        </Warning>
      </DialogContent>
    </BaseModal>
  );
