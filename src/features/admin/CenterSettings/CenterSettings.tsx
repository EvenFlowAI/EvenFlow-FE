...
const CenterSettings = () => {
  ...
  const {
    onOpen: onShowTimeOpen,
    isOpen: isShowTimeOpen,
    onClose: isShowTimeClose,
  } = useModal();
  const {
    onOpen: onServiceValetOpsCodeOpen,
  ...
  useEffect(() => {
    if (centerSettings?.dmsAppointmentTime) {
      ...
      setCalendarValue(
        dayjs().set("hour", +hour).set("minute", +min).set("second", +sec),
      );
    }
  }, [centerSettings]);
  ...
  const optContent: TOptContent = {
    [ECenterSettingType.ShowDropOffTime]: {
      helperText: "",
      label:
        centerSettings?.dropOffTimeDescription ??
        "User text (the text on the Booking Flow)",
    ...
    switch (k) {
      case ECenterSettingType.DmsAppointmentTime:
        return centerSettings?.dmsAppointmentTime
          ? dayjs
              .utc(centerSettings?.dmsAppointmentTime, "HH:mm:ss")
              .format("hh:mm a")
          : "Not Selected";
      default:
  ...
  const onChange = (date: TParsableDate) => {
    if (selectedSC) {
      const data: TDmsAppointmentTime = {
        dmsAppointmentTime: dayjs(date).format("HH:mm:ss"),
      };
      dispatch(
        updateDmsAppointmentTime(selectedSC.id, data, onClose, showError),
      );
    }
  };
