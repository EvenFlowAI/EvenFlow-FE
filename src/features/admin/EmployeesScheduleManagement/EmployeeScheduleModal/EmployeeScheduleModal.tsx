...
const EmployeeScheduleModal: React.FC<TProps> = ({
  ...
  const sorted = useMemo(
    () => [...scheduleByDate].sort(compareName),
    [scheduleByDate],
  );
  ...
  const handleShowOnBookingChange = (
  ...
  const checkIsValid = (): boolean => {
    ...
    if (!filtered.every((item) => item.finishAt && item.startAt)) {
      valid = false;
      showError(
        'Schedule for Employee that is "On Schedule" must not be empty',
      );
    }
    if (
      !filtered.every((item) =>
        dayjs(item.finishAt, timeSpanString).isAfter(
          dayjs(item.startAt, timeSpanString),
          "minute",
        ),
      )
    ) {
  ...
  const onSave = () => {
    ...
    if (checkIsValid()) {
      if (selectedSC && startDate && endDate) {
        ...
        const filteredData = filtered.map(
