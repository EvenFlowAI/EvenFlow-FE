...
const RoPredictionParameters = () => {
  const { selectedSC, predictionParams, predictionParamsLoading } = useSelector(
    (state: RootState) => state.serviceCenters,
  );
  const { selectedPod } = useSelector((state: RootState) => state.pods);

  const [isEdit, setEdit] = useState<boolean>(false);
  const [heavyRepairLaborHours, setHeavyRepairLaborHours] =
    useState<string>("0");
  const [otherRepairLaborHours, setOtherRepairLaborHours] =
    useState<string>("0");
  ...
  const handleSave = () => {
    if (
      heavyRepairLaborHours.match(fixedToTwo) &&
      ...
      if (selectedSC)
        dispatch(
          updatePredictionParams(selectedSC.id, data, onError, onSuccess),
        );
    } else {
    ...
    <div>
      ...
      {predictionParamsLoading ? (
        ...
        <DemandTable>
          ...
          <TableBody>
            <TableRow>
              <TableCell align="left">Heavy Repair Labor Hours</TableCell>
              <TableCell align="left">
                The number of incremental hours added to the appointment if the
                appointment is predicted a Heavy Repair
              </TableCell>
              <TableCell>
                {!isEdit ? (
            ...
            <TableRow>
              <TableCell align="left">Other Repair Labor Hours</TableCell>
              <TableCell align="left">
                The number of incremental hours added to the appointment if the
                appointment is{" "}
                <span style={{ textDecoration: "underline" }}>not</span>{" "}
                predicted a Heavy Repair
              </TableCell>
              <TableCell>
                {!isEdit ? (
            ...
            <TableRow>
              <TableCell align="left">Default Labor Hours</TableCell>
              <TableCell align="left">
                When an Open RO or an appointment booked outside of EvenFlow app
                uses Op Codes that are not in the Service Requests Page and the
                Labor Hour value cannot be found in the DMS
              </TableCell>
              <TableCell>
                {!isEdit ? (
                  defaultLaborHours
            ...
            <TableRow>
              <TableCell align="left">Pick Up & Drop Off Hours</TableCell>
              <TableCell align="left">
                The average labor hours for a pick up & drop off appointment
                used to determine the total daily appointment slot capacity
              </TableCell>
              <TableCell>
                {!isEdit ? (
                  pickUpDropOffHours
