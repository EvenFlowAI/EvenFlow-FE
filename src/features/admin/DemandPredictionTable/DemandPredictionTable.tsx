...
const DemandPredictionTable = () => {
  ...
  const handleSwitch =
    (
      ...
      requestType: "request" | "prediction",
    ) =>
    async (e: any, value: boolean) => {
      ...
      if (itemToUpdate) {
        const settingToUpdate = itemToUpdate.demandTypeSettings.find(
          (el) => el.type === type,
        );
        if (settingToUpdate) {
          const updated = {
            ...
            demandTypeSettings: itemToUpdate.demandTypeSettings
              ...
              .concat({
                ...
                isRequestStatusOn:
                  requestType === "request"
                    ? value
                    : settingToUpdate.isRequestStatusOn,
                isPredictedStatusOn:
                  requestType === "prediction"
                    ? value
                    : settingToUpdate.isPredictedStatusOn,
              }),
          };
              ...
              >
                ...
                <SwitchWrapperWhite key="ROs">
                  <Switch
                    onChange={handleSwitch(
                      item.podId,
                      EDemandPredictionType.OpenROs,
                      "request",
                    )}
                    checked={Boolean(
                      item.demandTypeSettings.find(
                        (el) => el.type === EDemandPredictionType.OpenROs,
                      )?.isRequestStatusOn,
                    )}
                    color="primary"
                  />
