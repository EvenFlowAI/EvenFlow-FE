...
const ServiceBookRowMobile: React.FC<TProps> = ({
  ...
  const isAdvisorSecondaryDisabled =
    advisorPrimaryMethod !== EAdvisorAssignMethod.LastEmployee;
  const isTechSecondaryDisabled =
    technicianPrimaryMethod !== EAdvisorAssignMethod.LastEmployee;
  ...
  return (
    <>
      <ServiceBook
        ...
        mdl={4}
        style={
          idx === data.length - 1 && !isOpened ? { borderBottomWidth: 1 } : {}
        }
      >
      ...
      {isOpened ? (
        <>
          ...
          <SmallGreyGrid
            ...
            mdl={2}
            style={
              isAdvisorSecondaryDisabled ? { backgroundColor: "#EAEBEE" } : {}
            }
          >
            Advisors Secondary
          </SmallGreyGrid>
          <SmallGreyGrid item xs={6} mdl={2} style={{ borderRightWidth: 0 }}>
            <Select
              ...
              onChange={onMethodChange(
                item,
                EAssignmentLevel.Primary,
                "Advisor",
              )}
              error={advisorPrimaryMethod === advisorSecondaryMethod}
              value={
                advisorOptions.find((el) => el.value === advisorPrimaryMethod)
                  ?.value ?? ""
              }
              input={<TextField />}
            >
          ...
          >
            <Select
              ...
              onChange={onMethodChange(
                item,
                EAssignmentLevel.Secondary,
                "Advisor",
              )}
              disabled={isAdvisorSecondaryDisabled}
              error={advisorPrimaryMethod === advisorSecondaryMethod}
              value={
                methodOptions.find((el) => el.value === advisorSecondaryMethod)
                  ?.value ?? ""
              }
              input={<TextField />}
            >
          ...
          <SmallGreyGrid
            ...
            mdl={2}
            style={
              isTechSecondaryDisabled ? { backgroundColor: "#EAEBEE" } : {}
            }
          >
            ...
            <Select
              ...
              value={
                technicianOptions.find(
                  (el) => el.value === technicianPrimaryMethod,
                )?.value ?? ""
              }
              input={<TextField />}
            >
          ...
          <SmallGreyGrid
            ...
            mdl={2}
            style={{
              backgroundColor: isTechSecondaryDisabled
                ? "#EAEBEE"
                : "transparent",
              borderBottomWidth: idx === data.length - 1 && isOpened ? 1 : 0,
            }}
          >
            <Select
              ...
              value={
                methodOptions.find(
                  (el) => el.value === technicianSecondaryMethod,
                )?.value ?? ""
              }
              input={<TextField />}
            >
