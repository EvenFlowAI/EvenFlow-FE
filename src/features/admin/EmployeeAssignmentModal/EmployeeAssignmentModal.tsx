...
> = (props) => {
  ...
  const isAdvisorSecondaryEnabled = useMemo(() => {
    return data.find(
      (item) =>
        item.employeeAssignmentSettings
          .find((el) => el.role === "Advisor")
          ?.methods?.find((el) => el.level === EAssignmentLevel.Primary)
          ?.type === EAdvisorAssignMethod.LastEmployee,
    );
  }, [data]);
  ...
  const onSave = () => {
    if (selectedSC) {
      const requestData: TUpdateAssignmentSettingsData = {
        ...
        serviceBookSettings: data.map(
          ({ employeeAssignmentSettings, serviceBookId }) => {
  ...
  const onChange = (
    ...
    value: EAdvisorAssignMethod | null,
  ) => {
    const itemToUpdate = data.find(
      (el) => el.serviceBookId === item.serviceBookId,
    );
    let roleToUpdate = itemToUpdate?.employeeAssignmentSettings.find(
      (el) => el.role === role,
    );
    let methodToUpdate = roleToUpdate?.methods?.find(
      (el) => el.level === level,
    );
    if (itemToUpdate && roleToUpdate) {
      methodToUpdate = methodToUpdate
        ...
        : { level, type: value };
      if (
        methodToUpdate.level === EAssignmentLevel.Primary &&
        value !== EAdvisorAssignMethod.LastEmployee
      ) {
        roleToUpdate = { ...roleToUpdate, methods: [methodToUpdate] };
      } else if (
        methodToUpdate.level === EAssignmentLevel.Secondary &&
        value === null
      ) {
        roleToUpdate = {
          ...roleToUpdate,
          methods: roleToUpdate.methods.filter(
            (el) => el.level !== EAssignmentLevel.Secondary,
          ),
        };
      } else {
        roleToUpdate = {
          ...roleToUpdate,
          methods: roleToUpdate.methods
            .filter((el) => el.level !== level)
            .concat(methodToUpdate),
        };
      }
      const newItem = {
        ...itemToUpdate,
  ...
  const onMobileMethodChange = useCallback(
    (
      ...
      (e: SelectChangeEvent<number>) => {
        if (item) {
          onChange(
            ...
            role,
            e.target?.value !== ""
              ? (+e.target?.value as EAdvisorAssignMethod)
              : null,
          );
        }
      },
    ...
    <BaseModal {...props} width={1100} onClose={onCancel}>
      ...
      <DialogContent style={isMobile ? { padding: "0px 16px" } : {}}>
        {loading ? (
          ...
          <EmployeeAssignmentMobile
            data={data}
            onMethodChange={onMobileMethodChange}
