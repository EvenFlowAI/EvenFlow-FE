...
const DayOfWeekOpsCode = () => {
  ...
  const { srPricingSettings, isLoading } = useSelector(
    ...
    if (assignedList.length && srPricingSettings) {
      setSelectedCodes(() => {
        return assignedList.filter((item) =>
          srPricingSettings.find((el) => el.serviceRequestId === item.id),
        );
      });
    }
    ...
    if (selectedSC && selectedCodes.length) {
      const data: TNewRequestsToPricing = {
        ...
        serviceRequestIds: selectedCodes
          ...
          .filter(
            (item) =>
              !srPricingSettings.find((el) => el.serviceRequestId === item),
          ),
      };
      try {
  ...
  const deleteOpsCode = useCallback(
    (item: TOpsCode) => {
      if (selectedSC) {
        askConfirm({
  ...
  const handleSelectOpsCode = useCallback(
        ...
        {isLoading ? (
          ...
          <DenseTable>
            ...
            <TableBody>
              {opsCodes.map((item) => {
                return (
                  <TableRow key={item.opsCode}>
                    <TableCell key={item.opsCode}>{item.opsCode}</TableCell>
                    <TableCell key="low">
                      <Slider
                        min={SliderRange.Min}
                        max={SliderRange.Max}
