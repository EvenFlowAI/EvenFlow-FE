...
const DayOfWeekPackage = () => {
  const { mpPricingSettings, isLoading } = useSelector(
  ...
  const deletePackageOption = useCallback(
    (item: TMPackage) => {
      if (selectedSC) {
        askConfirm({
          ...
          onConfirm: () => {
            try {
              dispatch(
                deletePackagePricingSettings(item.optionId, selectedSC.id),
              );
            } catch (e) {
              showError(e);
            }
              ...
              {mPackages.map((item) => {
                return (
                  <TableRow key={item.optionId}>
                    ...
                    <TableCell key={item.optionId}>{item.id}</TableCell>
