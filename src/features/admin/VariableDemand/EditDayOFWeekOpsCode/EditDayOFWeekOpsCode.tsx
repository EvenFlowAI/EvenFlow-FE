...
const EditDayOfWeekOpsCode: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<TEditDayOfWeekOpsCodeProps>>
> = ({ editingItem, ...props }) => {
  ...
  useEffect(() => {
    if (editingItem)
      setValues({ low: editingItem.low, high: editingItem.high });
  }, [editingItem]);
    ...
    if (selectedSC && editingItem) {
      ...
      if (values.low && data.values) {
        ...
        if (!values.low.toString().match(/(^-?\d*\.?\d{1,6}?)$/)) {
          return showError(
            "Value must be a number with maximum 6 decimal digits",
          );
        } else {
          data.values.push({
  ...
  return (
    <BaseModal {...props} width={340} onClose={onCancel}>
