...
> = ({ demand, type }) => {
  ...
  useEffect(() => {
    setForm({
      ...{
        [EDayDemand.High]:
          demand[EDayDemand.High]?.point || initialForm[EDayDemand.High],
        [EDayDemand.Low]:
          demand[EDayDemand.Low]?.point || initialForm[EDayDemand.Low],
      },
    });
  }, [demand]);
      ...
      try {
        await dispatch(
          setPricingDemand({
            ...
            point: form[t],
            demandCategory:
              EDemandCategory[String(t) as keyof typeof EDemandCategory],
            type,
          }),
        );
        setEdit(null);
          ...
          return (
            <TableRow key={d.id}>
              ...
              <TableCell align="right">
                {edit === d.id ? (
                  <>
                    <EditButton color="secondary" onClick={handleEdit(null)}>
                      CANCEL
                    </EditButton>
                    <EditButton color="primary" onClick={askSave(d.id)}>
                      SAVE
                    </EditButton>
                  </>
                ) : (
                  <EditButton color="primary" onClick={handleEdit(d.id)}>
                    EDIT
                  </EditButton>
                )}
              </TableCell>
