...
const tableData: TableRowDataType<IPackageOptionShort>[] = [
  {
    ...
    val: (el) =>
      el.maintenancePackageOptionName
        ? el.maintenancePackageOptionName.toString()
        : "-",
    align: "left",
  },
  {
    header: "PACKAGE ID",
    val: (el) => el.maintenancePackageId.toString(),
    align: "left",
  },
  {
    header: "PACKAGE NAME",
    val: (el) => el.maintenancePackageName,
    align: "left",
  },
];

const AddPackageToPricingModal: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<DialogProps>>
> = (props) => {
  const { mpOptionsList, mpPricingSettings } = useSelector(
  ...
  useEffect(() => {
    if (props.open) {
      ...
      setSelectedPackages(() => {
        return mpOptionsList.filter((el) =>
          mpPricingSettings
            .map((p) => p.maintenancePackageOptionId)
            .includes(el.maintenancePackageOptionId),
        );
      });
    }
      ...
      try {
        ...
        const data: TNewPackagesToPricing = {
          ...
          maintenancePackageOptionIds: selectedPackages
            ...
            .filter(
              (item) =>
                !mpPricingSettings.find(
                  (el) => el.maintenancePackageOptionId === item,
                ),
            ),
        };
  ...
  const handleSelect = (el: IPackageOptionShort) => {
    setSelectedPackages((prev) => {
      return prev.find(
        (item) =>
          item.maintenancePackageOptionId === el.maintenancePackageOptionId,
      )
        ? prev.filter(
            (item) =>
              item.maintenancePackageOptionId !== el.maintenancePackageOptionId,
      ...
      return (
        <Checkbox
          ...
          disabled={Boolean(
            mpPricingSettings.find(
              (item) =>
                item.maintenancePackageOptionId ===
                el.maintenancePackageOptionId,
            ),
          )}
          checked={Boolean(
