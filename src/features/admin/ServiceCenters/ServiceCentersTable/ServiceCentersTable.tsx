...
const rowDataSU: TableRowDataType<IServiceCenterExtended>[] = [
  {
    val: (el: IServiceCenterExtended) => el.dealership.name,
    header: "Dealership group",
  },
  {
    val: (el: IServiceCenterExtended) => el.name,
...
const rowDataA: TableRowDataType<IServiceCenterExtended>[] = [
  ...
  {
    val: (v) => v.countOfEmployees.toString(),
    header: "Employees",
    orderId: "countOfEmployees",
  },
  { val: (v) => v.id.toString(), header: "Service Center ID", width: 200 },
];
...
const ServiceCentersTable: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<TProps>>
> = ({ setEditedItem, editedItem, onOpen }) => {
  const {
    serviceCenters,
    loading,
  ...
  const viewActions = (el: IServiceCenterExtended) =>
    currentUser?.isSuperUser ? (
      ...
      <IconButton size="small" onClick={handleSetAnchor(el)}>
        <MoreHoriz />
      </IconButton>
    );
