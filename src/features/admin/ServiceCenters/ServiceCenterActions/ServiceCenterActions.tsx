...
export const ServiceCenterActions = () => {
  const search = useSelector(
    (state: RootState) => state.serviceCenters.searchTerm,
  );
  const { dealershipList } = useSelector(
    (state: RootState) => state.dealershipGroups,
  );
  const [selectedGroup, setSelectedGroup] = useState<TSelectedGroup | null>(
    null,
  );
  ...
  const onGroupChange = async (
    e: React.ChangeEvent<{}>,
    option: TSelectedGroup | null,
  ) => {
    setSelectedGroup(option);
    await dispatch(
      setSelectedDealershipGroupId(option?.id ? Number(option.id) : undefined),
    );
