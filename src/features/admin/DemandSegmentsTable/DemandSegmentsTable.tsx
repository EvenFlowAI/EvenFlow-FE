...
export const DemandSegmentsTable = () => {
  ...
  const { demandSegmentList: segments, listLoading: loading } = useSelector(
  ...
  useEffect(() => {
    setForm(
      segments.map((s) => [s.window1Point, s.window2Point, s.window3Point]),
    );
  }, [segments]);
  ...
  const handleChange =
    (idx: number, iIdx: number) => (e: React.ChangeEvent<HTMLInputElement>) => {
      const nForm = [...form];
      nForm[idx] = [...nForm[idx]];
      ...
      <TableBody>
        {loading ? (
          ...
          form.map((el, idx) => {
            return (
              <TableRow key={idx}>
                <EditingCell>{idx + 1}</EditingCell>
                {el.map((item, iIdx) => {
                  return (
                    <EditingCell key={`item-${iIdx}`}>
                      <TextField
                        id={`item-${iIdx}`}
                        value={item}
                        style={{ minWidth: 90 }}
          ...
          segments.map((segment, idx) => {
            return (
              <TableRow key={segment.id}>
                ...
                <TableCell>{segment.window1Point} %</TableCell>
                <TableCell>{segment.window2Point} %</TableCell>
                <TableCell>{segment.window3Point} %</TableCell>
                <TableCell />
              </TableRow>
            );
          })
