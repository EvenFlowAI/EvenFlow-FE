...
const CancelAppointmentModal: React.FC<
  React.PropsWithChildren<
    React.PropsWithChildren<
      DialogProps & { hashKey: string; loadData: TArgCallback<boolean> }
    >
  >
  ...
  const handleSubmit = () => {
    ...
    requestFunc(hashKey)
      .then(() => {
        ...
        loadData(
          Boolean(
            customerSearchData.firstName.length ||
              customerSearchData.lastName.length,
          ),
        );
        onClose();
      })
  ...
  const getTimeInfo = () => {
    return data?.serviceTypeOption?.type === EServiceType.PickUpDropOff ? (
      ...
      <span>
        ...
        {dayjs(data?.timeSlot, "hh:mm:ss").format("hh:mm a")} for customer{" "}
