...
const ReturningCustomerForAdmin: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<TProps>>
> = ({ handleNew, redirect }) => {
  const { customerEnteredEmail, scProfile } = useSelector(
    (state: RootState) => state.appointment,
  );
  const { customerSearchData, isLoading } = useSelector(
  ...
  const companyNameIsOn = useMemo(() => {
    return settings.find((el) => el.settingType === ESettingType.CompanyName)
      ?.data?.isOn;
  }, [settings]);
  ...
  const onError = (e: any) => {
    ...
    if (e.response?.data?.errors) {
      ...
      setErrors(() =>
        data
          .map((err: TError): string => err.field)
          .filter((el) => el !== null),
      );
    }
  };
  ...
  const handlePhoneOrEmailChange: React.ChangeEventHandler<
    HTMLInputElement
  > = ({ target: { value } }) => {
    setFormIsChecked(false);
    setErrors([]);
  ...
  const onTextChange =
    (name: keyof TCustomerSearchData) =>
  ...
  return (
    <Grid
      ...
      sm={12}
      md={6}
      style={{ maxWidth: 440, padding: isSm ? "16px 0" : 16 }}
    >
      <div className={classes.existing}>
        <span style={{ fontSize: isSm ? 28 : 32 }}>
          {t("Existing Customer")}
        </span>
        {isSm ? null : (
        ...
        <TextField
          ...
          error={
            formIsChecked && (errors.includes("PhoneOrEmail") || !formIsValid)
          }
          placeholder={t("Enter Phone or Email")}
          InputProps={{
            disableUnderline: true,
            endAdornment:
              isLoading && customerEnteredEmail.length ? (
                <LoadingProcess />
              ) : null,
          }}
          variant="standard"
        ...
        <div className={returningClasses.nameFieldsWrapper}>
          <TextField
            ...
            error={
              formIsChecked &&
              (customerSearchData.firstName.length === 1 || !formIsValid)
            }
            onChange={onTextChange("firstName")}
            InputProps={{
              disableUnderline: true,
              endAdornment:
                isLoading && customerSearchData.firstName.length ? (
                  <LoadingProcess />
                ) : null,
            }}
            fullWidth
          ...
          <TextField
            ...
            error={
              formIsChecked &&
              (customerSearchData.lastName.length === 1 || !formIsValid)
            }
            onChange={onTextChange("lastName")}
            InputProps={{
              disableUnderline: true,
              endAdornment:
                isLoading && customerSearchData.lastName.length ? (
                  <LoadingProcess />
                ) : null,
            }}
            fullWidth
        ...
        {isExpanded ? (
          <React.Fragment>
            {companyNameIsOn ? (
              <>
                ...
                <TextField
                  ...
                  error={
                    formIsChecked &&
                    (customerSearchData.companyName.length === 1 ||
                      !formIsValid)
                  }
                  ...
                  InputProps={{
                    disableUnderline: true,
                    endAdornment:
                      isLoading && customerSearchData.address.length ? (
                        <LoadingProcess />
                      ) : null,
                  }}
                  fullWidth
            ...
            <TextField
              ...
              error={
                formIsChecked &&
                (customerSearchData.address.length === 1 || !formIsValid)
              }
