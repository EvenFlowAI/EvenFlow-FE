...
> = ({ onComplete, loading, handleNew, redirect }) => {
  ...
  return (
    <div className={classes.wrapper}>
      <Grid className={classes.buttonsContainer} alignItems="stretch" container>
        {isAuthorized ? (
          <ReturningCustomerForAdmin
            handleNew={onSelectNewCustomer}
            redirect={redirect}
          />
        ) : (
      ...
      {isAuthorized && !!shortSC?.length && (
        <ActionButtons
          onBack={handleBack}
          onNext={() => {}}
          hideNext
          prevLabel="Change Service Center"
        />
      )}
    </div>
  );
