...
> = ({ selectedColumns, onClose, loadData, isNewVehicleMode, redirect }) => {
  ...
  const [sorting, setSorting] = useState<TSortOrder>({
    isAscending: true,
    order: null,
  });
  ...
  const setCustomerData = async (
    ...
    const customerData = customers.find(
      (el) =>
        el.vehicleId === item.vehicleId && el.customerId === item.customerId,
    );
    ...
    const vehicle = {
      ...
      model: item.model,
      year: item.year,
      appointmentHashKeys: item.appointmentHashKey
        ? [item.appointmentHashKey]
        : [],
    ...
    if (addressData?.fullAddress)
      await dispatch(setAddress(addressData.fullAddress));
    if (addressData?.zipCode)
      await dispatch(setZipCode(addressData.zipCode.slice(0, 5)));
      ...
      if (editingElement) {
        if (checkPhonesChanged()) {
          askConfirm({
            isRemove: false,
            title: t(
              "Please confirm the changes you made to the Customer Profile",
            ),
            onConfirm: () =>
              dispatch(
                updateCustomer(editingElement, onSuccess, (err) =>
                  showError(err),
                ),
              ),
          ...
          dispatch(
            updateCustomer(editingElement, onSuccess, (err) => showError(err)),
          );
        }
      }
    }
  ...
  const onSelectCustomerForNewVehicle =
    (customer: ICustomerWithPhones) => async () => {
      setFormChecked(false);
      ...
      if (customer?.address?.fullAddress)
        await dispatch(setAddress(customer.address.fullAddress));
    ...
    <div style={{ overflowX: "auto" }}>
      <div className={classes.tableWrapper}>
        <Table
          ...
          style={{
            width:
              selectedColumns.length > 10 ? selectedColumns.length * 150 : 1550,
          }}
        >
          <TableHead>
            <TableRow>
              ...
              {selectedColumns.map(({ name, order }, index) => {
                return (
                  <TableCell
                    key={name}
                    className={
                      index < 2 ? classes.stickyTHeadCell : classes.headerCell
                    }
                    style={{
                      left:
                        index === 0
                          ...
                          : index === 1
                            ...
                            : "unset",
                      borderRight:
                        index === 1 ? "1px solid #828282" : "1px solid #DADADA",
                    }}
          ...
          <TableBody>
            ...
            {data
              .slice(currentFirstItemIndex, currentLastItemIndex)
              .map((customer, index) => (
