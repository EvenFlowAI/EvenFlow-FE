...
const ServiceSelection: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<TServiceSelectionProps>>
> = ({ onNext, onBack }) => {
  const { valueService, serviceOffers, offersLoading } = useSelector(
  ...
  return (
    <OfferPageWrapper>
      <CarName>
        {valueService?.year?.year} {valueService?.series?.name}{" "}
        {valueService?.model?.name}
      </CarName>
      <ChangeButton onClick={onBack} variant="text">
        {t("Change Vehicle")}
      </ChangeButton>
            ...
            return (
              <OfferCardWrapper>
                ...
                <div className="buttonsWrapper">
                  <SelectButton onClick={onSelectClick(service)}>
                    {t("Select")}
                  </SelectButton>
