...
export const AppointmentReminders: React.FC<{ isEmailRequired: boolean }> = ({
  ...
  const { reminders, customer } = useSelector(
  ...
  const emailReminder = useMemo(() => {
    const reminder = reminders.find(
      (el) => el.toString() === EReminderType.Email.toString(),
    );
  ...
  useEffect(() => {
    if (customer?.email)
      dispatch(
        setReminders(Array.from(new Set([...reminders, EReminderType.Email]))),
      );
  }, [customer?.email]);
  ...
  return (
    <div>
      ...
      />
      <Info>
        {t("By checking the box, you agree to receive")}{" "}
        <strong>{t("text messages")}</strong>{" "}
      ...
      <StyledLabel
        ...
        control={
          <Checkbox
            disabled={
              !scProfile?.isSendReminders ||
              (!isEmailRequired && !customer?.email)
            }
