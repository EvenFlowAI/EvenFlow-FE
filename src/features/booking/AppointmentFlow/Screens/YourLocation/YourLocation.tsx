...
> = ({ onBack, onNext, isManagingFlow, restoreAddress }) => {
  ...
  const handleChangeAddress = async (e: any) => {
    if (!serviceOptionChangedFromSlotPage && !isManagingFlow)
      clearSelectedData();
  ...
  const loadAncillaryPrice = () => {
    if (address && zip.length && scProfile) {
      ...
      dispatch(
        loadAncillaryPriceByZip(data, onSuccess, showError, onUnavailableOpen),
      );
    }
  };
  ...
  const onInputChange = (e: React.ChangeEvent<{}>, value: string) => {
    if (scProfile) {
      ...
      dispatch(
        loadFilteredZip({ serviceCenterId: scProfile.id, search: value }),
      );
    }
  };
