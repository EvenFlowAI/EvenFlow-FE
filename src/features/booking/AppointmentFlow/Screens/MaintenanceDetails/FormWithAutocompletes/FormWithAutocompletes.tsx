...
const FormWithAutocompletes: React.FC<TFormProps> = ({
  ...
  const handleChange =
    (name: TKey, skip?: boolean) =>
    (e: React.ChangeEvent<{}>, option: string | null) => {
      ...
      if (option && !skip) {
        ...
        if (name === "make") {
          if (option === t("Other")) {
            ...
            if (option !== selectedVehicle?.make)
      ...
      <Grid item xs={12} sm={6} key="year" style={orderMapStyles.year}>
        <Autocomplete
          ...
          renderInput={autocompleteRender({
            label: t("Year"),
            placeholder: errors.includes("year")
              ? `${t("Year")} ${t("required")}`
              : `${t("Select")} ${t("Year")}`,
      ...
      <Grid item xs={12} sm={6} key="mileage" style={orderMapStyles.mileage}>
        <Autocomplete
          ...
          renderInput={autocompleteRender({
            ...
            placeholder: errors.includes("mileage")
