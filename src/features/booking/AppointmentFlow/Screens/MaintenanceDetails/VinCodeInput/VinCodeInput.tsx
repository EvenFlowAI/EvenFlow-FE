...
const VinCodeInput: React.FC<TProps> = ({
  ...
  const { selectedVehicle, userType } = useSelector(
  ...
  return recallsToggledOn ||
    ...
    <Grid
      ...
      style={{ display: "grid" }}
      className={
        recallsToggledOn && !isRecallsCategorySelected ? classes.vinWrapper : ""
      }
    >
      <TextField
        ...
        label={
          recallsToggledOn && !isRecallsCategorySelected
            ? t(
                "OPTIONAL: Please enter your VIN to check for open Safety Recalls",
              )
