...
> = ({
  ...
  const handleSelectChange =
    (name: TKey, skip?: boolean) => (e: SelectChangeEvent<unknown>) => {
      if (e.target.value && !skip) {
        ...
        if (name === "make") {
          if (e.target.value === t("Other")) {
            ...
            if (e.target.value !== selectedVehicle?.make)
              dispatch(updateVehicle({ model: "" }));
          }
        }
      }
    };
  ...
  return (
    <>
      ...
      <Grid item xs={12} sm={6} key="mileage" style={orderMapStyles.mileage}>
        ...
        <Select
          value={
            selectedVehicle?.mileage ? selectedVehicle.mileage.toString() : ""
          }
          className={classes.select}
          variant="standard"
          disableUnderline
      ...
      <Grid item xs={12} sm={6} key="make" style={orderMapStyles.make}>
        ...
        <Select
          value={
            selectedVehicle?.make ? selectedVehicle.make?.toLowerCase() : ""
          }
          className={classes.select}
          variant="standard"
          disableUnderline
        ...
        >
          ...
          {makeIsInTheList || !isExistingVehicle ? (
            ...
            <MenuItem
              disabled
              value={selectedVehicle?.make?.toLowerCase() ?? ""}
            >
      ...
      {currentConfig?.engineType ? (
        ...
        >
          <div className={classes.label}>
            {scProfile?.engineTypeFieldName ?? t("Engine Type")}*
          </div>
          <Select
            ...
            MenuProps={MenuProps}
            style={{
              color: selectedVehicle?.engineTypeId ? "inherit" : "#858585",
            }}
            disabled={
              Boolean(selectedEngine) &&
              Boolean(appointmentByKey?.vehicle?.engineTypeId)
            }
      ...
      <Grid item xs={12} sm={6} key="model" order={orderMapStyles.model.order}>
        ...
        <Select
          value={
            selectedVehicle?.model ? selectedVehicle.model.toLowerCase() : ""
          }
          className={classes.select}
          variant="standard"
          disableUnderline
