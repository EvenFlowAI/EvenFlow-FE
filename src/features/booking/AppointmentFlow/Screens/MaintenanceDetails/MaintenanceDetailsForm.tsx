...
> = ({ onBack, serviceCategoryPage, handleNext }) => {
  ...
  const { mileage, engineTypes } = useSelector(
  ...
  const [loadedOptions, setLoadedOptions] =
    useState<TOptionsState>(blankOptions);
  const [currentModels, setCurrentModels] = useState<string[] | []>([]);
  const [selectedEngine, setSelectedEngine] = useState<IEngineType | null>(
    null,
  );
  ...
  const recallsToggledOn = useMemo(
    () =>
      ...
      (currentConfig?.checkRecallsExisting && userType === EUserType.Existing),
    [currentConfig, userType],
  );

  const isRecallsCategorySelected = useMemo(() => {
    const isServiceRecall =
      service?.type == EServiceCategoryType.OpenRecalls &&
      service.page === EServiceCategoryPage.Page1;
    const isSubServiceRecall =
  ...
  useEffect(() => {
    ...
    const mileageIsValid =
      selectedMileage &&
      selectedMileage?.value.toString() ===
    ...
    if (selectedVehicle) {
      ...
      if (selectedMileage?.value) {
        const theSameMileageSelected =
          selectedVehicle?.mileage === selectedMileage?.value;
  ...
  useEffect(() => {
    if (!selectedVehicle?.make) {
      const defaultMake = makes.find(
        (item) => item.id === scProfile?.defaultVehicleMakeId,
      );
      if (defaultMake) {
        selectedVehicle &&
          dispatch(setVehicle({ ...selectedVehicle, make: defaultMake.name }));
  ...
  const removeRecallCategory = () => {
    if (
      ...
      dispatch(
        selectCategoriesIds(categoriesIds.filter((el) => el !== service.id)),
      );
    }
    if (
      subService?.type === EServiceCategoryType.OpenRecalls &&
      ...
      dispatch(
        selectCategoriesIds(categoriesIds.filter((el) => el !== subService.id)),
      );
    }
  };
  ...
  const isValid = () => {
    ...
    if (errorsArray.length) {
      const fields = errorsArray.map(
        (error) => error[0].toUpperCase() + error.slice(1),
      );
      const message = fields
        .join(", ")
        .concat(fields.length < 2 ? ` ${t("is")}` : ` ${t("are")}`)
        .concat(` ${t("required")}`);
      showError(message);
    }
    if (selectedVehicle?.vin && !checkVin(selectedVehicle.vin)) {
    ...
    if (
      ...
      if (checkVin(vin)) {
        if (
          vin &&
          make &&
          (recallsFromTheAdmin || isRecallsCategorySelected) &&
          year
        ) {
          setLoading(true);
          try {
  ...
  const orderMapStyles = {
    year: {
      order: isSM ? 2 : !currentConfig?.engineType && !recallsToggledOn ? 1 : 4,
    },
    mileage: {
      order: (currentConfig?.engineType || recallsToggledOn) && !isSM ? 1 : 3,
    },
    make: { order: 0 },
