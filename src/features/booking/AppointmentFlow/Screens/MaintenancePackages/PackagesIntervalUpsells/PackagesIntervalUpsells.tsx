...
const PackagesIntervalUpsells: React.FC<
...
> = ({
  ...
  return upsell.length ? (
    <React.Fragment>
      <div className="yellow subtitle">
        {title?.trim().length ? title : t("Dashboard Indicator Services")}
      </div>
      ...
      {upsell
        ...
        .map((c, i) => (
          <React.Fragment key={c.name + c.id + i}>
            <div
              className="serviceWithInfo"
              style={isBmWService ? { fontSize: 18 } : {}}
            >
              {c.name}{" "}
              {c.detailedDescription?.length ? (
                <HtmlTooltip
                  placement="right-end"
                  title={
                    <div
                      dangerouslySetInnerHTML={{
                        __html: c.detailedDescription,
                      }}
                    />
                  }
                >
