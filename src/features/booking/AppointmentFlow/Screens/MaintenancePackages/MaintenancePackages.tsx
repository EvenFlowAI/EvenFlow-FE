...
export const MaintenancePackages: React.FC<TPackageSelectionProps> = ({
  ...
  const [localSelectedPackage, setLocalSelectedPackage] =
    useState<IPackageOptions | null>(null);
    ...
    if (!scProfile?.eMenuEnabled) {
      ...
      Api.call<IPackage[]>(Api.endpoints.MaintenancePackages.ByVehicle, {
        ...
        .then(({ data }) => {
          setPackages(data);
          if (data.length)
            dispatch(setSelectedPackagePriceTitles(data[0].priceTitles));
        })
        .catch(() => {
          setPackages([]);
        })
  ...
  const handleClick =
    (p: IPackageOptions, pricing?: EPackagePricingType) => () => {
  ...
  const getTitle = useCallback(
    (type: EPackagePricingType) => {
      let title = "";
      if (loadedPackages[0]) {
        const price = loadedPackages[0].priceTitles?.find(
          (el) => el.type === type,
        );
    ...
    <PackagesStepWrapper>
        ...
        <React.Fragment>
          {isXs ? (
            ...
            <React.Fragment>
              ...
              {loadedPackages[0].priceTitles?.length &&
      ...
      {scProfile?.eMenuEnabled ? null : (
        <ActionButtons
          ...
          nextLabel={t("Next")}
          nextDisabled={
            !localSelectedPackage || localSelectedPricingType === null
          }
      ...
      <ConfirmChangeOption
        open={isOpen}
        onClose={handleDontChangeOption}
        onSave={onSave}
      />
      <AskAddService
        onSave={handleYes}
