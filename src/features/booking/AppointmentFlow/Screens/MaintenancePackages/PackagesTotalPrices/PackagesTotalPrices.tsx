...
const PackagesTotalPrices: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<TTotalProps>>
> = ({ isBmWService, packages, handleClick, isSanfordInfinity, setClasses }) => {
  ...
  return (
    <React.Fragment>
      ...
      {packages.map(p => {
        const priceBefore = p.marketPriceComplimentaryServices
        ...
        return (
          <div
            ...
            className={setClasses(
              p.id,
              `total ${scProfile?.isShowPriceDetails && !!priceBefore ? 'priceWithBefore' : 'price'} end`
            )}
            key={p.id}
          >
            ...
            <div className="currentWrp">
              ...
              <div className="current">
                ${scProfile?.isRoundPrice ? p.price : p.price.toFixed(2)}
              </div>
            </div>
          </div>
        );
