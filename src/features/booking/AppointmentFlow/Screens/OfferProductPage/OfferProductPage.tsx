...
> = ({ category, onChangeVehicle, handleSetScreen, lastCategory }) => {
  const { selectedVehicle, categoriesIds, subService, service, trackerData } =
  ...
  const handleCategoriesAndGA = () => {
    ...
    if (service && service.id === lastCategory?.id) {
      ...
      if (subService && subService?.id === lastCategory?.id) {
        if (
          categoriesIds &&
          subService.type !== EServiceCategoryType.LinkToPage2
        ) {
  ...
  return (
    <OfferPageWrapper>
      ...
      <PriceAndDate>
        <div className="innerWrapper">
          ...
          {category?.offer?.expiringDate ? (
            <div className="date">
              {t("Exp.date")}{" "}
              {dayjs(category.offer.expiringDate).format("MM/DD/YY")}
            </div>
          ) : null}
        </div>
      </PriceAndDate>
      {category?.offer?.description ? (
        <Description
          dangerouslySetInnerHTML={{ __html: category.offer.description }}
        />
      ) : null}
      <ActionButtons
        nextLabel={t("Next")}
