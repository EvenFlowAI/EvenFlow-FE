...
> = ({ card, active, onClick, onChangeTime, selectedTime, isLoading }) => {
  ...
  const { appointmentSlots } = useSelector(
      ...
      <div>
        <MobileDatePicker
          ...
          dayOfWeekFormatter={(day, date) =>
            dayjs(date as TParsableDate).format("ddd")
          }
          slotProps={{
            textField: {
              variant: "standard",
              InputProps: {
                endAdornment: (
                  <DateRangeIcon color={active ? "primary" : "disabled"} />
                ),
  ...
  return (
    <CardWrapper onClick={onClick} active={active} ref={cardRef}>
