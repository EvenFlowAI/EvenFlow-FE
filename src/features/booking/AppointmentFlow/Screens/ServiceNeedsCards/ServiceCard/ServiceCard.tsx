...
export const ServiceCard: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<TSCProps>>
> = ({ card, onSelect, active, selected }) => {
  const theme = useTheme();
  const isSM = useMediaQuery(theme.breakpoints.down("md"));
  const price =
    card.type === EServiceCategoryType.GeneralCategory ? card.price : undefined;

  return (
    <CardWrapper onClick={onSelect} selected={selected} active={active}>
      <CardDescription
