...
> = ({ serviceName, servicesList }) => {
  ...
  const isWaitList = useMemo(
    () =>
      ...
      (appointment?.isOverbookingApplied || appointmentByKey?.isWaitlist),
  ...
  const getDateForCalendarHeader = (): string[] => {
    ...
    if (isServiceValetApp) {
      dateFrom =
        dateFrom + serviceValetAppointment?.pickUpMin.split(":").join("");
  ...
  const getDateForCalendarDetails = useCallback(() => {
    ...
    if (isServiceValetApp) {
      ...
      const pickUpTime = `${t("Pick Up Time")}: ${dayjs
        .utc(serviceValetAppointment?.pickUpMin, timeSpanString)
        .format(
          time24HourFormat,
      ...
      if (
        ...
        if (pickUpMin && pickUpMax) {
          const pickUpTime = `${t("Pick Up Time")}: ${dayjs
            .utc(pickUpMin, timeSpanString)
            .format(
              time24HourFormat,
            )} ${t("to")} ${dayjs.utc(pickUpMax, timeSpanString).format(time24HourFormat)}`;
          dateString = dateString.concat("\n");
          dateString = dateString.concat(pickUpTime);
        }
      } else {
        dateString =
          dayjs(date).format(dateTimeString) ??
          dayjs.utc().format(dateTimeString);
      }
    }
    return dateString;
  }, [
  ...
  const calendarData: TItem[] = useMemo(() => {
    const data = [
      ...
      },
      {
        ...
        content: servicesList
          .map((item) =>
            item.includes("Going") ? t("My Description Of Need") : item,
          )
          .join(", "),
      },
      {
    ...
    if (serviceTypeOption?.type === EServiceType.MobileService) {
      const advisorIndex = data.findIndex(
        (el) => el.label === t("APPOINTMENT DETAILS"),
      );
