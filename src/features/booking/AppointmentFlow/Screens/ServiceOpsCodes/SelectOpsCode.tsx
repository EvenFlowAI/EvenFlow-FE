...
export const SelectOpsCode: React.FC<TProps> = ({
  ...
  const handleCategories = (value: string) => {
    const diagnoseCategory = allCategories.find(
      (item) =>
        item.type === EServiceCategoryType.Diagnose && item.page === page,
    );
    ...
    const individualCategory = allCategories.find(
      (item) =>
        item.type === EServiceCategoryType.IndividualServices &&
        item.page === page,
    );
    const individualRequestsIds =
      individualCategory?.serviceRequests.map((item) => item.id) || [];
    ...
    if (Number(value) && selectedSR.includes(Number(value))) {
      ...
      if (
        !filteredCodes.find((code) =>
          diagnoseCategoryRequestsIds.includes(code),
        )
      ) {
    ...
    <StepWrapper>
      <Wrapper>
            ...
            return (
              <CodeWrapper key={`${s.code} ${s.id}`}>
                ...
                <PricesWrapper>
                  ...
                  {Boolean(s.price) ? (
                    <Price>
                      ${scProfile?.isRoundPrice ? s.price : s.price.toFixed(2)}
                    </Price>
                  ) : (
                    <InfoOutlined
                      style={{ paddingRight: 8, fontSize: "2rem" }}
                    />
                  )}
                </PricesWrapper>
              </CodeWrapper>
        ...
        {allRequestsHavePrice ? null : (
          <Caption
            title={t(
              "The price for the service will be quoted at the dealership",
            )}
          />
        )}
      </Wrapper>
