...
const PackagesMobileTotalMaintenance: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<{ item: TPackage }>>
> = ({ item }) => {
  ...
  return scProfile?.isShowPriceDetails ? (
    <div className={classes.totalMaintenance}>
      <span className={classes.smallText}>{t("Total Maintenance Value")}:</span>
      <span className={classes.bigText}>
        $
        {scProfile?.isRoundPrice
          ? item.totalMaintenanceValue
          : item.totalMaintenanceValue.toFixed(2)}
      </span>
    </div>
  ) : null;
};
