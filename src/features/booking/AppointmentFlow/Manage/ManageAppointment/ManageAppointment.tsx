...
> = ({ onChangeSlot, onUpdateAppointment }) => {
  ...
  usePopState(
    isAuthorized ? "serviceCenterSelect" : "select",
  ...
  const handleConsultants = async () => {
    if (appointmentByKey) {
      await dispatch(
        loadConsultantsForUpdating(
          ...
          appointmentByKey?.serviceTypeOption
            ? appointmentByKey?.serviceTypeOption.id
            : null,
          appointmentByKey,
        ),
      );
      ...
      await dispatch(
        setAnyAdvisorSelected(appointmentByKey?.advisor?.isAnySelected ?? true),
      );
    }
  };
  ...
  const handleError = (e: any) => {
    const timeSlotUnavailable = e.response?.data?.message
      ?.toLowerCase()
      .includes("time slot");
    ...
    if (
      timeSlotUnavailable ||
      dateForZoneUnavailable ||
      transportationUnavailable
    ) {
