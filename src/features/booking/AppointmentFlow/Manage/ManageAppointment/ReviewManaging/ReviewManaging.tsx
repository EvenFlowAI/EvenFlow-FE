...
export const ReviewManaging = () => {
  ...
  const { currentConfig, isTransportationAvailable, isAdvisorAvailable } =
      ...
      <Wrapper>
        {transportationSelected ? (
          <li style={{ display: "flex" }}>
            Transportation needs: {transportationSelected?.description}
            {isTransportationAvailable ? (
              <Edit
                htmlColor="#142EA1"
                fontSize="small"
                onClick={handleChangeTransportation}
        ...
        {currentConfig?.advisorSelection ? (
          <li style={{ display: "flex" }}>
            {t("Service Advisor")}: {advisor?.name ?? t("Any Available")}
            {isAdvisorAvailable ? (
              <Edit
                htmlColor="#142EA1"
                fontSize="small"
