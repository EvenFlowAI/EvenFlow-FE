...
> = ({ open, onClose }) => {
  ...
  const price = useMemo(() => {
    if (
      ...
      return serviceValetAppointment?.price?.value &&
      ...
      <DialogContent>
                ...
                <li className={classes.item} key={item.requestName}>
                  ...
                  <div className={classes.pricesBlock}>
                    {item.offer ? (
                      <span
                        className={
                          item?.offer?.type === EOfferType.FreeService
                            ? classes.offersText
                            : classes.offersPrice
                        }
                      >
                        {getOfferString(
                          item.offer,
                          Boolean(scProfile?.isRoundPrice),
                        )}
                      </span>
                    ) : null}
                    {Object(item).hasOwnProperty("priceValue") &&
                    item.priceValue &&
                    item.pricingDisplayType !==
                      EPricingDisplayType.Suppressed ? (
                      <span className={classes.price}>
                        $
                        {scProfile?.isRoundPrice
                          ? item.priceValue
        ...
        {noDefinedPriceExists && (
          <DetailedFeesInfo>
            <ErrorOutline />
            <span className="text">
              {t("Service item will be quoted at dealership")}
            </span>
          </DetailedFeesInfo>
        )}
      </DialogContent>
