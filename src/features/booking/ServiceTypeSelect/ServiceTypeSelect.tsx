...
> = ({ handleValueServiceConfig, loading }) => {
  const {
    trackerData,
    userType,
    selectedVehicle,
  ...
  const scId = useMemo(
    () => (id ? id : scProfile?.id ? encodeSCID(scProfile.id) : ""),
    [scProfile, id],
  );
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down("mdl"));
  const isTagLinePresent = useMemo(
    () => firstScreenOptions.some((el) => el.taglineText),
    [firstScreenOptions],
  );

  const setTracker = (ids: string[]) =>
    dispatch(setTrackerCreated({ isCreated: true, ids }));
  ...
  const redirect = () => {
    if (scId) {
      customerLoadedData?.isUpdating
        ? history.push(
            Routes.EndUser.ManageAppointmentFrame.replace(":id", scId),
          )
  ...
  const handleVisitCenterSwitch = (newServiceOption: IFirstScreenOption) => {
    const newConfigHasTransportation = config.find(
      (el) => el.serviceType === newServiceOption.type,
    )?.transportationNeeds;
  ...
  const handleUpdateOption = (serviceOption: IFirstScreenOption) => {
    const requestDataIsValid =
      serviceOption?.type === EServiceType.VisitCenter ||
      Boolean(address && zipCode);
    requestDataIsValid &&
      dispatch(
        checkCarIsValid(() => dispatch(loadConsultants(id, serviceOption.id))),
      );
  ...
  const onServiceTypeSelect = (newServiceOption: IFirstScreenOption) => {
    ...
    if (customerLoadedData?.isUpdating) {
      ...
      const nextScreen =
        newServiceOption.type === EServiceType.VisitCenter
          ? "serviceNeeds"
          : "location";
              ...
              return (
                <div key={card.id}>
                  <ServiceTypeButton
                    ...
                    isTaglinePresent={
                      isMobile ? !!card.taglineText?.length : isTagLinePresent
                    }
                  >
                    {card.description ? (
                      <HtmlTooltip
                        ...
                        title={
                          <div style={isMobile ? { maxWidth: 280 } : {}}>
                      ...
                      >
                        <div className="infoIcon">
                          <InfoOutlined style={{ color: "#828282" }} />
                        </div>
                      </HtmlTooltip>
                    ) : null}
                    <div className={classes.name}>{card.name}</div>
                    {(isMobile && card.taglineText) || isTagLinePresent ? (
                      <Tagline taglineColor={card.taglineFontColorHex}>
                        {card.taglineText}
                      </Tagline>
                    ) : null}
