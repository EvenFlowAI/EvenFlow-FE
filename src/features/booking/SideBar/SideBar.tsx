...
> = ({ screen, handleSetScreen, isManagingFlow }) => {
  const {
    sideBarSteps,
    sideBarMenu,
    sideBarActualSteps,
  ...
  const getStepsState = useCallback(
    (idx: number): boolean => {
  ...
  const getButtonState = useCallback(
    (index: number) => {
      ...
      if (sideBarActualSteps) {
        ...
        return (
          currentScreenNumberValue < index + 1 &&
          lastPassedScreenNumberValue < index + 1
        );
      }
      return false;
    },
            ...
            return (
              <li key={item}>
                <Button
                  ...
                  color="primary"
                  style={
                    !getButtonState(idx) && !getStepsState(idx + 1)
                      ? activeButtonStyles
                      : {}
                  }
