...
> = ({ onNext, onBack }) => {
  ...
  const yearOptions = useMemo(
    () => seriesModels.map((item) => item.year.toString()),
    [seriesModels],
  );
  ...
  const onSeriesChange = (e: React.ChangeEvent<{}>, option: TSeries | null) => {
    ...
    dispatch(
      setValueServicePartial({ series, model: null, selectedService: null }),
    );
        ...
        <SelectWrapper>
          ...
          {valueService?.year && currentSeries?.length ? (
            <Autocomplete
              ...
              isOptionEqualToValue={(option) =>
                option.id === valueService?.series?.id
              }
              disableClearable
              autoComplete={true}
              className={classes.input}
          ...
          {valueService?.year && valueService?.series ? (
            <Autocomplete
              ...
              getOptionLabel={(option) => option.name}
              isOptionEqualToValue={(option) =>
                option.name === valueService?.model?.name
              }
              autoComplete={true}
              className={classes.input}
              renderInput={autocompleteRender({
