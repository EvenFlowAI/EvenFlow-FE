...
const SwitchFlowModal: React.FC<TProps> = ({
  ...
  const [transportationOption, setTransportationOption] =
    useState<ITransportation | null>(null);
  ...
  const newConfig = config.find(
    (item) => item.serviceType === selectedOption?.type,
  );
  ...
  useEffect(() => {
    ...
    const zipIsSelectedByUser =
      zip === zipCodeValue && zipCodeValue?.length === 5;
  ...
  const handleNextStep = () => {
    ...
    dispatch(
      updateAppointmentDetails({
        ...
        advisor: consultant,
        date:
          timingType === EAppointmentTimingType.PreferredDate
            ? selectedTime
            : null,
        timing: timingType,
        transportation:
  ...
  return (
    <BaseModal open={open} onClose={onCancel} width={700}>
      <DialogTitle
        onClose={onCancel}
        style={{ fontSize: 24, padding: "16px 36px 0 36px" }}
      >
        ...
        <Grid container>
          {isAddressVisible ? (
            <>
              <Grid item xs={12}>
                <TextWrapper>
                  {t("Where do you want to be picked up?")}
                </TextWrapper>
              </Grid>
              <Grid item xs={12}>
                <UserLocation
          ...
          {isAdvisorVisible ? (
            <>
              <Grid item xs={12}>
                <TextWrapper>
                  {t("Do you have a preferred Service Advisor?")}
                </TextWrapper>
              </Grid>
          ...
          {isTransportationsVisible ? (
            <>
              <Grid item xs={12}>
                <TextWrapper>
                  {t("Do you need assistance with transportation?")}
                </TextWrapper>
              </Grid>
              <Grid item xs={12} sm={6}>
                <Transportation
          ...
          {isDateSelectionOn ? (
            <>
              <Grid item xs={12}>
                <TextWrapper>
                  {t("When would you like your vehicle serviced?")}
                </TextWrapper>
              </Grid>
              <Grid item xs={12} sm={6}>
                <Timing
