...
export const EditAppointment = () => {
  ...
  const isFromAdmin = useMemo(() => {
    const isFromAdmin = new URLSearchParams(search)
      .get("fromAdmin")
      ?.toLowerCase();
  ...
  const setFrameScreen = (
    serviceTypeOption: IFirstScreenOption | undefined,
  ) => {
    dispatch(
      setCurrentFrameScreen(
  ...
  useEffect(() => {
    ...
    requestFunc(id)
      .then(async ({ data }) => {
        ...
        if (selectedScId)
          history.push(
            Routes.EndUser.ManageAppointmentFrame.replace(
              ":id",
              encodeSCID(selectedScId),
            ),
          );
      })
      .catch((e) => {
  ...
  const handleCreateNew = () => {
    if (selectedScId) {
      ...
      if (customerLoadedData) {
        dispatch(
          setCustomerLoadedData({ ...customerLoadedData, isUpdating: false }),
        );
      }
      history.replace(
        `${Routes.EndUser.Welcome}/${encodeSCID(selectedScId)}?frame=1`,
      );
    }
  };
