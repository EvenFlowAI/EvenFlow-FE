...
export const Welcome = () => {
  ...
  const { shortLoading } = useSelector(
  ...
  const redirect = () => {
    const route = isFrame
      ? Routes.EndUser.AppointmentFrame
      : Routes.EndUser.Appointment;
    if (id) {
  ...
  const handleValueServiceConfig = (serviceType: EServiceType) => {
    ...
    if (currentConfig)
      dispatch(setValueServiceAvailability(currentConfig.valueService));
  ...
  const getTitle = (view: TView) => {
    return view === "serviceCenterSelect"
      ...
      : view === "serviceSelect"
        ? t("Do you want to bring your car in")
        : null;
  };
  const getSubTitle = (view: TView) => {
    return view === "serviceSelect"
      ? t("Or use our mobile service?")
      : t("schedule service");
  };
  ...
  return !scProfile || isProfileLoading || shortLoading ? (
    ...
    <React.Fragment>
      <WelcomeLayout
        title={getTitle(welcomeScreenView)}
        subtitle={getSubTitle(welcomeScreenView)}
      >
        {/*<LanguageSwitcher/>*/}
        {getComponent()}
        <ExistingCustomerError
