...
export const CancelAppointment = () => {
  const [appointment, setAppointment] = useState<IAppointmentByQuery | null>(
    null,
  );
  ...
  const handleCreateNew = () => {
    if (appointment?.serviceCenterId) {
      clearStorage();
      history.replace(
        `${Routes.EndUser.Welcome}/${encodeSCID(appointment.serviceCenterId)}?frame=1`,
      );
    }
  };
    ...
    switch (tState) {
      ...
      case "new":
        return (
          <div>
            <p>
              {t("Please confirm you want to cancel your appointment for")}{" "}
            ...
            <Info>
              {t("If you've changed your mind - you can close this window")}.
            </Info>
          </div>
        );
      default:
  ...
  return (
    <WelcomeLayout title={""}>
      <ContentContainer>{getData()}</ContentContainer>
    </WelcomeLayout>
  );
};
