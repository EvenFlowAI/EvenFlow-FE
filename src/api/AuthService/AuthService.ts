...
class AuthService {
  ...
  async refresh() {
    try {
      ...
      const resp = await Api.call<ITokens>(
        Api.endpoints.Authentications.Refresh,
        { data },
      );
  ...
  async dealershipLogin(dealershipId: number) {
    try {
      const { data: tokens } =
        await API.authentication.dealership(dealershipId);
  ...
  async login(data: ICredentials) {
    const resp = await Api.call<ITokens>(
      Api.endpoints.Authentications.Request,
      { data },
    );
    this.setTokens(resp.data);
    this.refreshRequest();
