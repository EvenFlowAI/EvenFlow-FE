...
const accounts = {
  passwordRecovery: (
    data: IPasswordRecoveryData,
  ): TApiResponse<IPasswordRecoveryResp> =>
...
const appointment = {
  list: (
    data: IListAppointmentRequest,
  ): TApiResponse<PaginatedAPIResponse<IAppointment>> =>
    request.post("/appointments/by-query", data),
  cancelByKey: (key: string): TApiResponse =>
    request.put(`/appointments/${key}/cancel/by-key`),
  cancel: (id: number): TApiResponse =>
...
const employeeSchedules = {
  remove: (id: number): TApiResponse<{}> =>
    request.delete(`/employee-schedules/${id}`),
};
