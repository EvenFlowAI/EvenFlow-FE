...
type TProps = {
  ...
  selectFirstSlot?: (
    date?: TParsableDate,
    newSeviceOption?: IFirstScreenOption,
  ) => void;
};
...
> = ({ date, loading, appointments, selectFirstSlot }) => {
  ...
  const handleSelect = useCallback(
    (a: IRemappedAppointmentSlot | null) => {
      const data =
        a && selectedTiming ? { ...a, timingType: selectedTiming } : a;
      handleGA(a);
  ...
  const generateSlots = (
    ...
    while (dayjs(cDate).isSameOrBefore(end, "minute")) {
      slots.push({
        date: dayjs.utc(cDate),
        label: dayjs.utc(cDate).format("h:mm a"),
      });
  ...
  const slots: TSlot[] = useMemo(() => {
    let slots: TSlot[] = [];
    const currentSCSchedule = hoursOfOperations.find(
      (item) => item.dayOfWeek === dayjs(date).day(),
    );
    if (gap) {
