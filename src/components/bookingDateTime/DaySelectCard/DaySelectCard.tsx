...
export const DaySelectCard: React.FC<TProps> = ({ day, onClick, appointment, isCurrent, isXs }) => {
  ...
  return (
    <DayCard available={isAvailable} isCurrent={isCurrent} isOffPeak={isOffPeak} onClick={!isAvailable ? () => {} : onClick}>
      <Date>{utcDay.format(monthDayFormat)}</Date>
      <Day available={isAvailable} isCurrent={isCurrent} isOffPeak={isOffPeak} onClick={!isAvailable ? () => {} : onClick}>
