...
export const PickUpSlotCard: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<TProps>>
> = ({ timeSlot, onSelect, selected, date }) => {
  ...
  return (
    <PickUpWrapper
      key={
        timeSlot ? dayjs(timeSlot.date).toISOString() : dayjs().toISOString()
      }
      ...
      <div className={classes.rightPart}>
        <div className={classes.availability}>
          {timeSlot && timeSlot.available > 0 ? (
            <div
              className={classes.availabilityItem}
              style={{ color: "#008331" }}
            >
