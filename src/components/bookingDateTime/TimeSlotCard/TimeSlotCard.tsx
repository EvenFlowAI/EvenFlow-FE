...
export const TimeSlotCard: React.FC<TProps> = ({
  ...
  const { waitListSettings } = useSelector(
  ...
  const isWaitList = Boolean(
    slot?.isOverbookingApplied && waitListSettings?.isEnabled,
  );
  ...
  useEffect(() => {
    if (isTodaySlot) {
      const differenceInMSeconds = dayjs(
        dayjs(slot?.date).format("YYYY-MM-DDTHH:mm:ss"),
      ).diff(dayjs.utc());
      if (differenceInMSeconds > 0) {
        everySlotTimoutId = setTimeout(
          () => setTimePassed(true),
