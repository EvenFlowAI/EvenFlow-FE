...
export const SVDaySelector: React.FC<
...
> = ({
  ...
  const [daysInMonth, days]: [number, string[]] = useMemo(() => {
    ...
    if (searchedDateRange) {
      ...
      let i = 0;
      const maxAvailableDaysAmount =
        daysInMonth < WHILE_LIMIT ? WHILE_LIMIT : daysInMonth;
      while (
        currentDate.isSameOrBefore(endDate, "date") &&
        i < maxAvailableDaysAmount
      ) {
        generatedDays.push(
  ...
  const handlePrev = () => {
    if (prevAvailable()) {
      ...
      if (
        selectedTiming === EAppointmentTimingType.PreferredDate &&
        !isAdminPanel
      ) {
        onOpen();
      }
      ...
      {days.slice(sliceIdx, sliceIdx + daysPerScreen).map((day) => (
        <SVDaySelectCard
          ...
          appointment={appointments.find((item) =>
            dayjs(item.date).isSame(dayjs(day), "date"),
          )}
