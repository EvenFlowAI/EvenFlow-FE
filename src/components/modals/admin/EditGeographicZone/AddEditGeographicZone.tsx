...
import {
  ...
  TZonesServiceType,
  TZoneUpdate,
} from "../../../../store/reducers/mobileService/types";
...
> = ({
  ...
  const {
    ...
    zones: mobileZones,
  } = useSelector((state: RootState) => state.mobileService);
  const {
    currentZone: currentServiceValetZone,
    isLoading: isValetLoading,
    zones: valetZones,
  } = useSelector((state: RootState) => state.serviceValet);
  const { filteredZipCodes } = useSelector(
  ...
  const zonesList = useMemo(
    () => (serviceType === "serviceValet" ? valetZones : mobileZones),
    [serviceType, valetZones, mobileZones],
  );
  ...
  const onInputChange = (e: React.ChangeEvent<{}>, value: string) => {
    if (selectedSC) {
      dispatch(
        loadFilteredZip({ serviceCenterId: selectedSC.id, search: value }),
      );
    }
  };
    ...
    if (zoneName.length && selectedSC) {
      if (currentZone && isEdit) {
        ...
        if (zipList.length) {
          ...
          if (serviceType === "serviceValet") {
            dispatch(
              addServiceValetZone(selectedSC.id, data, onSuccess, onError),
            );
          } else {
            dispatch(
              addMobServiceZone(selectedSC.id, data, onSuccess, onError),
            );
          }
        } else {
  ...
  const onChangeZoneClick = (code: string) => {
    if (setCurrentZip && isEdit && currentZone) {
      const codeObject = currentZone.zipCodes.find(
        (item) => item.code === code,
      );
      if (codeObject) {
        ...
        showError(
          "This code is not saved to the Zip codes list of the current zone",
        );
      }
    }
  };
          ...
          {isMobileLoading || isValetLoading ? (
            ...
            <>
              ...
              <div className={classes.zipsWrapper}>
                {zipList.map((code) => (
                  <div className={classes.zip} key={code}>
