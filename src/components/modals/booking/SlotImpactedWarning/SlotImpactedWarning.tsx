...
const SlotImpactedWarning = () => {
  const { isSlotsWarningOpen } = useSelector(
    (state: RootState) => state.modals,
  );
  const { isAppointmentTimingAvailable, isTransportationAvailable } =
  ...
  return (
    <BaseModal width={450} open={isSlotsWarningOpen} onClose={onCancel}>
      <DialogTitle onClose={onCancel}>
        {t(
          "Appointment availability depends on your selections. Please continue to see options.",
        )}
      </DialogTitle>
