...
const ServiceImpactedWarning = () => {
  const { isServiceWarningOpen } = useSelector(
    (state: RootState) => state.modals,
  );
  const { appointmentByKey } = useSelector(
  ...
  return (
    <BaseModal width={450} open={isServiceWarningOpen} onClose={onCancel}>
      <DialogTitle onClose={onCancel}>
        <div>
          {t(
            "The services our Mobile Truck offer may be different from our Service Center.",
          )}
        </div>
        <div>
          {t("Please continue to see available services and appointment times")}
        </div>
      </DialogTitle>
