...
import {getAppointmentList} from "./mock";
...
export const ListAppointmentSelection = () => {
    ...
    return <div>
        {appointments.map(({id, date, offer, earlyDropOff, shortWait, loanerCar}) =>
            <Box key={id} mt={.5}>
                <Appointment variant="outlined">
                    ...
                    <Button color="primary" fullWidth variant="outlined">Schedule</Button>
                </Appointment>
            </Box>
        )}
    </div>
