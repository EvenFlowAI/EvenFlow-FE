...
const TimeSelect: React.FC<TProps> = ({
  ...
  const upEnabled = useMemo(() => {
    return dayjs(value, timeSpanString).isBefore(
      dayjs(end, timeSpanString),
      "minute",
    );
  }, [value, end]);

  const downEnabled = useMemo(() => {
    return dayjs(value, timeSpanString).isAfter(
      dayjs(start, timeSpanString),
      "minute",
    );
  }, [value, start]);
  ...
  const onPeriodChange = (period: TDayPeriod) => {
    ...
    onChange(
      dayjs(`${pureHours} ${period}`, time12HourFormat).format(timeSpanString),
    );
  };
  ...
  const onAutocompleteChange = (
    ...
    if (option) {
      onChange(
        dayjs(`${option} ${period}`, time12HourFormat).format(timeSpanString),
      );
    } else {
      onChange("");
    }
