...
export const setCurrentMake = createAction<IMake | null>(
  "VehicleDetails/SetCurrentMake",
);
...
export const setPodsMakes = createAction<IMakeExtended[]>(
  "VehicleDetails/MakesModels",
);
export const getEngineType = createAction<IEngineType[]>(
...
export const updateMake =
  ...
  async (dispatch, getState) => {
    const { selectedSC } = getState().serviceCenters;
    if (selectedSC) {
      Api.call(Api.endpoints.Vehicles.UpdateMake, {
        urlParams: { id: makeId },
        data,
      })
        .then((result) => {
...
export const loadMileage =
  ...
  async (dispatch) => {
    dispatch(setLoading(true));
    Api.call<IMileage[]>(Api.endpoints.Vehicles.GetMileage, {
...
export const loadMakesForPods =
  (serviceCenterId: number): AppThunk =>
  (dispatch) => {
    Api.call<IMakeExtended[]>(Api.endpoints.Vehicles.MakesModels, {
      params: { serviceCenterId },
    })
      .then((result) => {
...
export const loadEngineType =
  ...
  async (dispatch) => {
    dispatch(setLoading(true));
    Api.call(Api.endpoints.Vehicles.GetEngineType, {
      params: { serviceCenterId },
    })
      .then((result) => {
        if (result?.data) {
