...
export const setProfileLoading = createAction<boolean>(
  "Appointment/SetProfileLoading",
);
export const setTopAligning = createAction<boolean>(
  "Appointment/SetTopAligning",
);
...
export const loadSRs =
  ...
  async (dispatch, getState) => {
    try {
      const {
        data: { result },
      } = await Api.call<PaginatedAPIResponse<ISR>>(
...
export const selectSRMultiple = createAction<number[]>(
  "Appointment/SelectSRMultiple",
);
export const handleSearch = createAction<string>("Appointment/Search");
export const changeReminders = createAction<Partial<IReminders>>(
  "Appointment/ChangeReminders",
);
export const changePrivacy = createAction<Partial<IPrivacy>>(
  "Appointment/ChangePrivacy",
);
export const changePersonalInformation = createAction<
...
export const selectServiceValetAppointment =
  createAction<IServiceValetAppointment | null>(
...
export const setSlotPodId = createAction<number | null>(
  "Appointment/SetSlotPodId",
);
export const setSlotsLoading = createAction<boolean>(
  "Appointment/setSlotsLoading",
);
...
export const loadAppointmentSlots =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      } = await Api.call<IAppointmentResponse>(
      ...
      const res = dispatch(
        getAppointmentSlots(
          items.map((item) => ({
            ...item,
            searchDate: data.fromDate as TParsableDate,
          })),
        ),
      );
...
export const setLoadedReducer = createAction<TAppointmentState>(
  "Appointment/ReloadState",
);
...
export const setOldAppointmentId = createAction<
  ICreateAppointmentResp & { updated: boolean }
>("Appointments/SetAppointmentId");
export const setAppointmentFilters = createAction<Partial<IAppointmentFilters>>(
  "Appointment/SetFilters",
);
export const setCustomerEnteredEmail = createAction<string>(
  "Appointment/SetCustomerEnteredEmail",
);
...
export const setEditAppointment = createAction<TAppointmentState>(
  "Appointment/SetEditAppointment",
);
...
export const getDropOffSettings = createAction<IDropOffSettings>(
  "Appointment/GetDropOffSettings",
);
export const loadServiceValetSlots =
  ...
  (dispatch) => {
    dispatch(setSlotsLoading(true));
    Api.call<ISVAppointmentResponse>(
      Api.endpoints.AppointmentSlots.GetServiceValetSlots,
      { data },
    )
      .then((result) => {
        ...
        dispatch(
          getServiceValetSlots(
            items.map((el) => ({ ...el, uniqueId: uuidv4() })),
          ),
        );
