...
export const loadAll = (): AppThunk => (dispatch, getState) => {
  ...
  Api.call<PaginatedAPIResponse<IDealershipGroupExtended>>(
    Api.endpoints.Dealerships.GetAll,
    { data },
  )
    .then((result) => {
...
> =
  ...
  async (dispatch) => {
    ...
    const mappedData = {
      contactPerson: {
        ...data.contactPerson,
        phoneNumber: data.contactPerson.personPhoneNumber,
      },
      dealership: {
        ...data.dealership,
        phoneNumber: data.dealership.dealershipPhoneNumber,
      },
    };
...
const _loadDealershipProfile = (
  payload: IDealershipProfile,
): DealershipActions => ({ type: "Dealership/Profile", payload });
export const loadDealershipProfile = (): AppThunk => async (dispatch) => {
  try {
    const { data } = await Api.call<IDealershipProfile>(
      Api.endpoints.Accounts.Dealership,
    );
...
export const updateDealership =
  ...
  async (dispatch) => {
    try {
      await Api.call(Api.endpoints.Dealerships.Update, {
        urlParams: { id },
        data: payload,
      });
...
export const updateDealershipAvatar =
  ...
  async (dispatch) => {
    try {
      ...
      await Api.call(Api.endpoints.Dealerships.UploadAvatar, {
        urlParams: { id },
        data,
      });
      onSuccess && onSuccess();
