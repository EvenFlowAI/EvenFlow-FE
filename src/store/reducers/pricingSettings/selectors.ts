...
export const calculationsSelector = (state: RootState) =>
  state.pricingSettings.calculations;
...
export const demandSelector = (state: RootState) =>
  state.pricingSettings.pricingDemands;
...
const dowSelector = (state: RootState) => state.pricingSettings.dWeekPricing;
export const mappedDWeekPricingSelector = createSelector(dowSelector, (items) =>
  items.reduce(
    (acc, item) => {
      acc[item.dayOfWeek] = item.demandCategory;
      return acc;
    },
    {} as TEnumValueMap<EDay, EDemandCategory>,
  ),
);
