...
export const _changePageData = changePageDataGeneric(
  "Employees/ChangePageData",
);
...
export const loadAll: ActionCreator<AppThunk> =
  () => async (dispatch, getState) => {
    ...
    try {
      const {
        data: { result: employees, paging },
      } = await Api.call<PaginatedAPIResponse<IEmployee>>(
  ...
  async (dispatch) => {
    ...
    try {
      const { data } = await Api.call<IEmployee | string>(
        Api.endpoints.Employees.Create,
        { data: payload },
      );
      if (avatar) {
        await dispatch(
...
export const updateEmployee =
  ...
  async (dispatch) => {
    dispatch(saving(true));
    try {
      await Api.call(Api.endpoints.Employees.Update, {
        urlParams: { id },
        data,
      });
      if (avatar) {
...
const loadDealership = (payload: boolean): TEmployeeActions => ({
  type: "Employees/LoadingDealership",
  payload,
});
const _loadDealership = (payload: IEmployee[]): TEmployeeActions => ({
  type: "Employees/GetDealershipEmployees",
  payload,
});
export const loadDealershipEmployees =
...
export const getSCAdvisors = createAction<IAdvisorShort[]>(
  "SCEmployees/GetAdvisors",
);
export const getSCEmployees = createAction<IAdvisorShort[]>(
  "SCEmployees/GetEmployees",
);
export const loadSCAdvisors =
  (serviceCenterId: number): AppThunk =>
  async (dispatch) => {
    try {
      const {
        data: { result },
      } = await Api.call<PaginatedAPIResponse<IAdvisorShort>>(
...
export const setEmplOrder = createAction<IOrder<IEmployee>>(
  "SCEmployees/SetOrder",
);

export const getDMSAdvisors = createAction<TServiceConsultant[]>(
  "SCEmployees/GetDMSAdvisors",
);
export const loadingDMSAdvisors = createAction<boolean>(
  "SCEmployees/LoadingDMSAdvisors",
);
      ...
      const {
        ...
        Api.endpoints.Users.GetAll,
        {
          data: {
            ...
            ...state.employees.order,
            searchTerm: state.employees.searchTerm,
            roles: state.employees.filters.role
              ? [state.employees.filters.role]
              : [],
...
export const setEmployeeFilters = createAction<Partial<IEmployeeFilters>>(
  "Employees/ChangeFilters",
);

export const getUsersShort = createAction<IAdvisorShort[]>(
  "Employees/GetUsersShort",
);
export const loadUsersShort =
  (serviceCenterId: number): AppThunk =>
  (dispatch) => {
    dispatch(loading(true));
    Api.call(Api.endpoints.Users.GetShort, {
...
export const getBaseSummary = createAction<IBaseSummary>(
  "Employees/GetBaseSummary",
);
export const loadBaseSummary =
  (serviceCenterId: number, orderBy: string, isAscending = true): AppThunk =>
  (dispatch) => {
    dispatch(loading(true));
    Api.call<IBaseSummary>(Api.endpoints.EmployeeSchedule.GetBaseSummary, {
      data: { serviceCenterId, isAscending, orderBy },
    })
      .then((result) => {
        if (result) {
...
export const getBaseSummaryByEmployee = createAction<IEmployeeRoleHours[]>(
  "Employees/GetBaseScheduleByEmployee",
);
export const loadBaseSummaryByEmployee =
  (data: TScheduleByEmployeeRequestData): AppThunk =>
  (dispatch) => {
    dispatch(loading(true));
    Api.call<IBaseScheduleByEmployee>(
      Api.endpoints.EmployeeSchedule.GetSummaryByEmployee,
      { data },
    )
      .then((result) => {
        if (result) {
...
export const getBaseEmployeeSchedule = createAction<IEmployeeSchedule>(
  "Employees/GetScheduleTimeByEmployee",
);
export const loadBaseEmployeeSchedule =
  (serviceCenterId: number, employeeId: string): AppThunk =>
  (dispatch) => {
    const data: TBaseScheduleRequest = { serviceCenterId, employeeId };
    dispatch(loading(true));
    Api.call<IEmployeeSchedule>(
      Api.endpoints.EmployeeSchedule.GetTimeScheduleByEmployee,
      { params: data },
    )
      .then((res) => {
...
export const updateBaseEmployeeSchedule =
  ...
  (dispatch) => {
    ...
    Api.call(Api.endpoints.EmployeeSchedule.SetTimeScheduleByEmployee, { data })
      .then(() => {
        dispatch(
          loadBaseEmployeeSchedule(data.serviceCenterId, data.employeeId),
        );
        onSuccess();
      })
      .catch((err) => {
...
export const loadAssignmentSettings =
  (serviceCenterId: number): AppThunk =>
  (dispatch) => {
    dispatch(loading(true));
    Api.call(Api.endpoints.Employees.GetAssignmentSettings, {
      urlParams: { serviceCenterId },
    })
      .then((result) => {
