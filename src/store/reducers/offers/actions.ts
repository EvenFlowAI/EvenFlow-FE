...
export const setOffersPageData =
  createAction<Partial<IPageRequest>>("Offers/PageData");
...
export const getArchivedOffers = createAction<IOffer[]>(
  "Offers/GetArchivedOffers",
);
export const setArchivedOffersPageData = createAction<Partial<IPageRequest>>(
  "Offers/ArchivedPageData",
);
export const setArchivedOffersPaging = createAction<IPagingResponse>(
  "Offers/ArchivedPaging",
);
export const setArchivedOffersLoading = createAction<boolean>(
  "Offers/ArchivedLoading",
);
...
export const loadOffers =
  ...
  async (dispatch, getState) => {
    ...
    try {
      ...
      const {
        data: { result, paging },
      } = await Api.call<PaginatedAPIResponse<IOffer>>(
...
export const loadArchivedOffers =
  (serviceCenterId: number): AppThunk =>
  async (dispatch, getState) => {
...
export const updateOffer =
  (data: IOfferForm, archive?: boolean): AppThunk =>
  async (dispatch) => {
    try {
      await Api.call(Api.endpoints.Offers.Edit, {
        data,
        urlParams: { id: data?.id || 0 },
      });
      if (archive) {
...
export const removeOffer =
  (offer: IOffer, archive?: boolean): AppThunk =>
  async (dispatch) => {
    try {
      await Api.call(Api.endpoints.Offers.ChangeStatus, {
        urlParams: { id: offer.id },
        data: { status: EOfferStatus.Deleted },
      });
      if (archive) {
...
export const setArchiveOffer =
  (data: IOffer, archive?: boolean): AppThunk =>
  async (dispatch) => {
    try {
      await Api.call(Api.endpoints.Offers.ChangeStatus, {
        data: {
          status:
            data.status === EOfferStatus.Archived
              ? EOfferStatus.None
