...
export const getAppointments = createAction<IAppointment[]>(
  "Appointments/GetAppointments",
);
export const getAllAppointments = createAction<IAppointment[]>(
  "Appointments/GetAllAppointments",
);
export const setAppointmentsLoading = createAction<boolean>(
  "Appointments/SetAppointmentsLoading",
);
export const setAppointmentsModalLoading = createAction<boolean>(
  "Appointments/SetAppointmentsModalLoading",
);
export const setAppointmentsCount = createAction<number>(
  "Appointments/SetAppointmentsCount",
);
export const setAllAppointmentsCount = createAction<number>(
  "Appointments/SetAllAppointmentsCount",
);
export const getPackageByVehicle = createAction<IPackageAppointments[]>(
  "Appointments/GetPackageByVehicle",
);
export const getServiceBookList = createAction<TServiceBook[]>(
  "Appointments/GetServiceBookList",
);
export const getScheduler = createAction<TScheduler[]>(
  "Appointments/GetSchedulerList",
);
...
export const getTechnicians = createAction<TServiceConsultant[]>(
  "Appointments/GetTechnicians",
);
...
export const loadAppointments =
  ...
  (dispatch) => {
    ...
    API.appointment
      ...
      .finally(() =>
        setTimeout(() => dispatch(setAppointmentsLoading(false)), 500),
      );
  };
      ...
      .then((result) => {
        if (result?.data) {
          dispatch(
            getAdvisorsList(
              result.data.filter(
                (item) => item.role === EConsultantRole.Advisor,
              ),
            ),
          );
          dispatch(
            getTechnicians(
              result.data.filter(
                (item) => item.role === EConsultantRole.Technician,
              ),
            ),
          );
        }
...
const loadSlotsForCloning =
  ...
  (dispatch, getState) => {
    ...
    const advisorId = consultants.find(
      (item) => item.id === currentAppointment?.advisor?.id,
    )?.id;
    if (currentAppointment) {
      const data: IAppointmentSlotsRequest = {
        ...
        serviceCenterId,
        advisorId:
          !currentAppointment?.advisor?.isAnySelected && advisorId
            ? advisorId
            : null,
        fromDate: dayjs().startOf("day").add(utcOffset, "minute").toISOString(),
        maintenancePackageOption:
          currentAppointment.maintenancePackageOption ?? null,
      ...
      if (currentAppointment.address?.zipCode)
        data.zipCode = currentAppointment.address?.zipCode;
      ...
      if (currentAppointment.driver?.email)
        data.searchTerm = currentAppointment.driver?.email;
      if (
        currentAppointment.serviceTypeOption?.type ===
    ...
    try {
      ...
      if (data) {
        const mileageIsValid =
          ...
          mileage.find(
            (item) =>
              item.value.toString() === data?.vehicle?.mileage?.toString(),
          );
