...
export const getProximity = createAction<IProximity[]>(
  "SlotScoring/GetProximity",
);
export const loadProximity =
...
export const createProximity =
  (data: IProximity): AppThunk =>
  async (dispatch) => {
    dispatch(setLoading(true));
    try {
      await Api.call(Api.endpoints.SlotScoring.SetProximity, { data });
...
export const getDesirability = createAction<IDesirability[]>(
  "SlotScoring/GetDesirability",
);
export const loadDesirability =
  (
    serviceCenterId: number,
    dayOfWeek: EDesirabilityDays | null,
    podId?: number,
    errorCallback?: (err: { errorCode: number; message: string }) => void,
  ): AppThunk =>
  async (dispatch) => {
    dispatch(setLoading(true));
    let params =
    ...
    Api.call<IDesirability[]>(Api.endpoints.SlotScoring.GetDesirability, {
      params,
    })
      .then(({ data }) => {
...
export const getOptimizationSettings = createAction<IOptimizationSetting[]>(
  "SlotScoring/GetOptimizationSettings",
);
export const loadOptimizationSettings =
  (serviceCenterId: number, podId?: number): AppThunk =>
  async (dispatch) => {
    dispatch(setLoading(true));
    try {
...
export const setOptimizationSettings =
  (data: IOptimizationSettingsCreateForm): AppThunk =>
  async (dispatch) => {
    try {
...
export const setSettingValues =
  (
    data: IOptimizationSettingValueForm,
    serviceCenterId: number,
    podId?: number,
  ): AppThunk =>
  async (dispatch) => {
...
export const loadRange =
  ...
  (dispatch) => {
    const data =
      dayOfWeek !== null
        ? { dayOfWeek, serviceCenterId, podId }
        : { serviceCenterId, podId };
...
export const loadHoursOfOperations =
  ...
  (dispatch) => {
    ...
    Api.call<IHOODataForm[]>(Api.endpoints.ServiceCenters.GetHOO, {
      ...
      .then((result) => {
        if (result?.data) {
          const startTimes = result.data.map((item) =>
            dayjs(item.from, "HH:mm:ss"),
          );
          const endTimes = result.data.map((item) =>
