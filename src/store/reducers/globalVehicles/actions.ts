...
export const getAllMakes = createAction<IGlobalMake[]>(
  "GlobalVehicles/GetAllMakes",
);
export const getModels = createAction<IGlobalModel[]>(
  "GlobalVehicles/GetModels",
);
export const getAllModels = createAction<IGlobalModel[]>(
  "GlobalVehicles/GetAllModels",
);
export const setLoading = createAction<boolean>("GlobalVehicles/Loading");
export const setPaging = createAction<IPagingResponse>(
  "GlobalVehicles/SetMakesPaging",
);
export const setModelsPaging = createAction<IPagingResponse>(
  "GlobalVehicles/SetModelsPaging",
);
export const getMakesStatistics = createAction<TVehicleStatistic>(
  "GlobalVehicles/GetMakesStatistics",
);
export const getModelsStatistics = createAction<TVehicleStatistic>(
  "GlobalVehicles/GetModelsStatistics",
);

export const loadGlobalMakes =
  (
  ...
  (dispatch) => {
    ...
    Api.call<PaginatedAPIResponse<IGlobalMake>>(
      Api.endpoints.GlobalVehicles.GetMakes,
      {
        data: {
          pageIndex,
          pageSize,
...
export const loadAllGlobalMakes = (): AppThunk => (dispatch) => {
  ...
  Api.call<PaginatedAPIResponse<IGlobalMake>>(
    Api.endpoints.GlobalVehicles.GetMakes,
    { data: { pageIndex: 0, pageSize: 0, isAscending: true } },
  )
    .then((res) => {
      if (res?.data?.result) {
...
export const loadGlobalModels =
  ...
  (dispatch) => {
    dispatch(setLoading(true));
    const { pageIndex, pageSize } = pageData;
    Api.call<PaginatedAPIResponse<IGlobalModel>>(
      Api.endpoints.GlobalVehicles.GetModels,
      {
        data: {
          pageIndex,
          pageSize,
  ...
  (dispatch) => {
    ...
    Api.call(Api.endpoints.GlobalVehicles.UpdateModels, { data: { items } })
      .then((res) => {
        if (res) {
          dispatch(
            loadGlobalModels(pageData, order, reviewStatus, makeIds, modelIds),
          );
