...
export const getSettings = createAction<IDemandPrediction[]>(
  "DemandManagement/GetSettings",
);
...
export const loadDemandManagementSettings =
  ...
  (dispatch) => {
    ...
    Api.call(Api.endpoints.DemandManagement.GetSettings, {
    ...
    Api.call(Api.endpoints.DemandManagement.UpdateSettings, { data })
      .then((res) => {
        if (data.serviceCenterId)
          dispatch(loadDemandManagementSettings(data.serviceCenterId));
      })
      .catch((err) => {
