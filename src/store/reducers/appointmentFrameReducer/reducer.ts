  ...
  builder
    ...
    .addCase(setTiming, (state, { payload }) => {
      return {
        ...
        selectedTime:
          payload !== EAppointmentTimingType.PreferredDate
            ? null
            : state.selectedTime,
      };
    })
    ...
    .addCase(setMaintenanceDetails, (state, { payload }) => {
      return {
        ...state,
        maintenanceDetails: { ...state.maintenanceDetails, ...payload },
      };
    })
    .addCase(setUpdateAppointment, (state, { payload }) => {
      return {
        ...
        serviceType:
          payload.serviceTypeOption?.type ?? EServiceType.VisitCenter,
        ...
        packagePricingType:
          payload?.maintenancePackageOption?.priceType ?? null,
      };
    })
    ...
    .addCase(setServiceTypeOption, (state, { payload }) => {
      ...
      const optionIsInTheList = state.selectedServiceOptions.find(
        (el) => el.id === payload?.id,
      );
      return {
        ...state,
    ...
    .addCase(deleteLastScreen, (state) => {
      const screens = state.passedScreens.slice(
        0,
        state.passedScreens.length - 1,
      );
      return {
        ...state,
        passedScreens: screens,
    ...
    .addCase(setFiltersVisibility, (state, { payload }) => {
      return {
        ...state,
        filtersVisibility: { ...state.filtersVisibility, ...payload },
      };
    })
