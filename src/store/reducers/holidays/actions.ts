...
const loading = (payload: boolean): THolidayActions => ({
  type: "Holidays/Loading",
  payload,
});
...
export const setHolidayPageData = (
  payload: Partial<IPageRequest>,
): THolidayActions => ({ type: "Holidays/ChangePageData", payload });
const _loadAll = (payload: IHoliday[]): THolidayActions => ({
  type: "Holidays/LoadAll",
  payload,
});
export const loadAllHolidays =
  ...
  async (dispatch, getState) => {
    ...
    try {
      const {
        data: { paging, result },
      } = await Api.call<PaginatedAPIResponse<IHoliday>>(
...
export const getWeeklyHolidaysList = createAction<IHoliday[]>(
  "Holidays/GetFullList",
);
