...
const useTransportationVisibility = () => {
  ...
  const someServicesHaveDefaultTransportation = useMemo(() => {
    ...
    return (
      someOptionHasDefaultTransportation &&
      someOptionHasNotDefaultTransportation
    );
  }, [firstScreenOptions]);

  const noOneServiceHasTransportation =
    firstScreenOptions.filter((el) => !el.transportationOption).length ===
    firstScreenOptions.length;

  const isTransportationsVisible = useMemo(() => {
    const transportationExists = transportations.find(
      (el) =>
        el.id === transportation?.id ||
        el.id === serviceTypeOption?.transportationOption?.id,
    );
