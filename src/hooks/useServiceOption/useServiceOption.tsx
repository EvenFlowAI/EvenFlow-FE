...
export const useServiceOption = (optionType: 'serviceType' | 'transportation') => {
  ...
  const onRedirectToLocation = (option: IFirstScreenOption, showAdvisorScreen: boolean) => {
    ...
    const shouldRedirectToLocation =
      !address || !zipCode || !serviceOptionChangedFromSlotPage || !optionWasSelectedPreviously;
    if (address && zipCode) {
  ...
  return (newOption: IFirstScreenOption) => {
    ...
    try {
      ...
      isTransportationAvailable =
        Boolean(newConfig?.transportationNeeds) && !newOption?.transportationOption;
      if (newOption?.type === EServiceType.PickUpDropOff || !newConfig?.appointmentSelection)
